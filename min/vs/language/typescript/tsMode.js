...
define('vs/language/typescript/tsMode', ['require', 'require'], (require) => {
	var moduleExports = (() => {
		...
		var ne = (s, e, t) =>
			e in s ? I(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (s[e] = t);
		var $ = ((s) =>
			...
			V = (s, e, t, i) => {
				if ((e && typeof e == 'object') || typeof e == 'function')
					for (let a of ie(e))
						!se.call(s, a) &&
							a !== t &&
							I(s, a, { get: () => e[a], enumerable: !(i = te(e, a)) || i.enumerable });
				return s;
			},
				...
				constructor(t, i, a, u) {
					...
					let g = (l) => {
							...
							let f = () => {
									...
									k ? l.isAttachedToEditor() && this._doValidate(l) : this._doValidate(l);
								},
								d,
						...
						p = (l) => {
							...
							this._listener[f] && (this._listener[f].dispose(), delete this._listener[f]);
						};
...
`
				...
				_convertRelatedInformation(t, i) {
					...
					return (
						i.forEach((u) => {
							let g = t;
							if ((u.file && (g = this._libFiles.getOrCreateModel(u.file.fileName)), !g)) return;
							let p = u.start || 0,
								o = u.length || 1,
			...
			v = class extends S {
								...
								suggestions: f.entries.map((m) => {
									...
									if (m.replacementSpan) {
										let C = e.getPositionAt(m.replacementSpan.start),
											U = e.getPositionAt(m.replacementSpan.start + m.replacementSpan.length);
										h = new r.Range(C.lineNumber, C.column, U.lineNumber, U.column);
									}
									let k = [];
									return (
				...
				async resolveCompletionItem(e, t) {
					let i = e,
						...
						u = i.position,
						g = i.offset,
						o = await (await this._worker(a)).getCompletionEntryDetails(a.toString(), g, i.label);
					return o
						? {
								uri: a,
								position: u,
...
${X(i)}`;
		...
		function X(s) {
			...
			if (s.name === 'param' && s.text) {
				let [t, ...i] = s.text;
				(e += `\`${t.text}\``), i.length > 0 && (e += ` \u2014 ${i.map((a) => a.text).join(' ')}`);
			} else
							...
							(d.documentation = { value: x(f.documentation) }),
								...
								f.parameters.forEach((m, h, k) => {
									let C = x(m.displayParts),
										U = { label: C, documentation: { value: x(m.documentation) } };
									(d.label += C),
										d.parameters.push(U),
			...
			M = class extends S {
				...
				async provideDefinition(t, i, a) {
					...
					if (
						!o ||
						t.isDisposed() ||
						(await this._libFiles.fetchLibFilesIfNecessary(o.map((f) => r.Uri.parse(f.fileName))),
						t.isDisposed())
					)
						return;
					let l = [];
			...
			A = class extends S {
				...
				async provideReferences(t, i, a, u) {
					...
					if (
						!l ||
						t.isDisposed() ||
						(await this._libFiles.fetchLibFilesIfNecessary(l.map((d) => r.Uri.parse(d.fileName))),
						t.isDisposed())
					)
						return;
					let f = [];
...
`,
			...
			O = class extends w {
				...
				async provideDocumentRangeFormattingEdits(e, t, i, a) {
					...
					if (e.isDisposed()) return;
					let l = await o.getFormattingEditsForRange(u.toString(), g, p, w._convertOptions(i));
...
`
					...
					for (let g of i.changes)
						for (let p of g.textChanges)
							a.push({
								resource: e.uri,
								versionId: void 0,
								textEdit: { range: this._textSpanToRange(e, p.span), text: p.newText }
							});
					return {
						title: i.description,
						edit: { edits: a },
			...
			W = class extends S {
				...
				async provideRenameEdits(t, i, a, u) {
					...
					if (f.canRename === !1) return { edits: [], rejectReason: f.localizedErrorMessage };
					if (f.fileToRename !== void 0) throw new Error('Renaming files is not supported.');
		...
		function Y(s, e) {
			...
			function p() {
				...
				Z(i),
					o.completionItems && i.push(r.languages.registerCompletionItemProvider(e, new v(u))),
					o.signatureHelp && i.push(r.languages.registerSignatureHelpProvider(e, new _(u))),
					...
					o.documentSymbols && i.push(r.languages.registerDocumentSymbolProvider(e, new R(u))),
					o.rename && i.push(r.languages.registerRenameProvider(e, new W(g, u))),
					o.documentRangeFormattingEdits &&
						i.push(r.languages.registerDocumentRangeFormattingEditProvider(e, new O(u))),
