...
export let SuggestController = class SuggestController {
	...
	constructor(
		...
		this.widget = this._toDispose.add(
			new IdleValue(() => {
				...
				this._toDispose.add(
					widget.onDidSelect((item) => this._insertSuggestion(item, 0 /* InsertFlags.None */), this)
				);
				...
				const commitCharacterController = new CommitCharacterController(
					...
					widget,
					this.model,
					(item) => this._insertSuggestion(item, 2 /* InsertFlags.NoAfterUndoStop */)
				);
				...
				this._toDispose.add(
					widget.onDidFocus(({ item }) => {
						...
						let value = true;
						if (
							this.editor.getOption(1 /* EditorOption.acceptSuggestionOnEnter */) === 'smart' &&
			...
			this.model.onDidSuggest((e) => {
				...
				if (e.triggerOptions.auto) {
					...
					if (
						...
						((_a = SnippetController2.get(this.editor)) === null || _a === void 0
							? void 0
							: _a.isInSnippet())
					) {
	...
	_insertSuggestion(event, flags) {
		...
		const info = this.getOverwriteInfo(
			item,
			Boolean(flags & 8 /* InsertFlags.AlternativeOverwriteConfig */)
		);
		// keep item in memory
		...
		snippetController.insert(insertText, {
			...
			undoStopAfter: false,
			adjustWhitespace: !(
				(item.completion.insertTextRules & 1) /* CompletionItemInsertTextRule.KeepWhitespace */
			),
	...
	_reportSuggestionAcceptedTelemetry(item, model, acceptedSuggestion) {
		...
		const providerId = item.extensionId
			? item.extensionId.value
			: ((_a = acceptedSuggestion.item.provider._debugDisplayName) !== null && _a !== void 0
					? _a
					: 'unknown'
			  )
					.split('(', 1)[0]
	...
	getOverwriteInfo(item, toggleMode) {
		...
		const overwriteAfter =
			(replace ? item.editReplaceEnd.column : item.editInsertEnd.column) - item.position.column;
			...
			this.model.onDidSuggest(
				({ completionModel }) => {
					...
					this._insertSuggestion(
						...
						4 /* InsertFlags.KeepAlternativeSuggestions */ |
							1 /* InsertFlags.NoBeforeUndoStop */ |
							2 /* InsertFlags.NoAfterUndoStop */
					);
				},
	...
	new SuggestCommand({
		...
		handler: (x, arg) => {
			x.triggerSuggestAndAcceptBest(
				isObject(arg) ? Object.assign({ fallback: 'tab' }, arg) : { fallback: 'tab' }
			);
		},
		kbOpts: {
			weight,
...
registerEditorAction(
	class extends EditorAction {
		...
		run(_accessor, editor) {
			var _a;
			(_a = SuggestController.get(editor)) === null || _a === void 0
				? void 0
				: _a.resetWidgetSize();
		}
	}
