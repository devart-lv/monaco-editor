...
registerColor(
	...
	{
		dark: transparent(editorSuggestWidgetForeground, 0.5),
		light: transparent(editorSuggestWidgetForeground, 0.5),
		hcDark: transparent(editorSuggestWidgetForeground, 0.5),
		hcLight: transparent(editorSuggestWidgetForeground, 0.5)
	},
	nls.localize(
...
class PersistedWidgetSize {
	...
	restore() {
		var _a;
		const raw =
			(_a = this._service.get(this._key, 0 /* StorageScope.PROFILE */)) !== null && _a !== void 0
				? _a
				: '';
		try {
...
export let SuggestWidget = class SuggestWidget {
	constructor(editor, _storageService, _contextKeyService, _themeService, instantiationService) {
			...
			this.element.onDidResize((e) => {
				...
				if (state) {
					...
					if (!state.persistHeight || Math.abs(state.currentSize.height - height) <= threshold) {
						height =
							(_b = (_a = state.persistedSize) === null || _a === void 0 ? void 0 : _a.height) !==
								null && _b !== void 0
								? _b
								: defaultSize.height;
					}
					if (!state.persistWidth || Math.abs(state.currentSize.width - width) <= threshold) {
						width =
							(_d = (_c = state.persistedSize) === null || _c === void 0 ? void 0 : _c.width) !==
								null && _d !== void 0
								? _d
								: defaultSize.width;
					}
		...
		const applyIconStyle = () =>
			this.element.domNode.classList.toggle(
				'no-icons',
				!this.editor.getOption(113 /* EditorOption.suggest */).showIcons
			);
		applyIconStyle();
		...
		const applyStatusBarStyle = () =>
			this.element.domNode.classList.toggle(
				'with-status-bar',
				this.editor.getOption(113 /* EditorOption.suggest */).showStatusBar
			);
		applyStatusBarStyle();
	...
	_onListFocus(e) {
		...
		if (item !== this._focusedItem) {
			...
			this._currentSuggestionDetails = createCancelablePromise((token) =>
	...
	toggleDetails() {
		if (this._isDetailsVisible()) {
			...
			(this._state === 3 /* State.Open */ ||
				this._state === 5 /* State.Details */ ||
				this._state === 4) /* State.Frozen */
		) {
		...
		if (this._state === 2 /* State.Empty */ || this._state === 1 /* State.Loading */) {
			...
			this._cappedHeight =
				height === fullHeight
					? {
							wanted:
								(_c = (_b = this._cappedHeight) === null || _b === void 0 ? void 0 : _b.wanted) !==
									null && _c !== void 0
									? _c
									: size.height,
							capped: height
	...
	_positionDetails() {
		...
		if (this._isDetailsVisible()) {
			this._details.placeAtAnchor(
				this.element.domNode,
				((_a = this._contentWidget.getPosition()) === null || _a === void 0
					? void 0
					: _a.preference[0]) === 2 /* ContentWidgetPositionPreference.BELOW */
			);
		}
	}
	getLayoutInfo() {
		...
		const itemHeight = clamp(
			this.editor.getOption(115 /* EditorOption.suggestLineHeight */) || fontInfo.lineHeight,
			8,
			1000
		);
