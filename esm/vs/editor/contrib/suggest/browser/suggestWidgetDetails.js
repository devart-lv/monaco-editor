...
export let SuggestDetailsWidget = class SuggestDetailsWidget {
	...
	getLayoutInfo() {
		const lineHeight =
			this._editor.getOption(115 /* EditorOption.suggestLineHeight */) ||
			this._editor.getOption(48 /* EditorOption.fontInfo */).lineHeight;
	...
	renderItem(item, explainMode) {
		...
		if (explainMode) {
			...
			md += `word: ${
				item.completion.filterText ? item.completion.filterText + ' (filterText)' : item.textLabel
			}\n`;
			...
			md += `commit_chars: ${
				(_b = item.completion.commitCharacters) === null || _b === void 0 ? void 0 : _b.join('')
			}\n`;
...
export class SuggestDetailsOverlay {
		...
		this._disposables.add(
			this.widget.onDidChangeContents(() => {
				...
				if (this._anchorBox) {
					this._placeAtAnchor(
						this._anchorBox,
						(_a = this._userSize) !== null && _a !== void 0 ? _a : this.widget.size,
	...
	_placeAtAnchor(anchorBox, size, preferAlignAtTop) {
		...
		const placement =
			(_a = placements.find((p) => p.fit >= 0)) !== null && _a !== void 0
				? _a
