	...
	extract(item, out) {
		...
		if (typeof item.completion.documentation === 'string') {
			...
			if (
				match &&
				(match.index === 0 ||
					match.index + match[0].length === item.completion.documentation.length)
			) {
				out[0] = match[0];
				return true;
			}
...
export let ItemRenderer = class ItemRenderer {
	...
	renderTemplate(container) {
		...
		const configureFont = () => {
			...
			const lineHeight =
				options.get(115 /* EditorOption.suggestLineHeight */) || fontInfo.lineHeight;
		...
		disposables.add(
			this._editor.onDidChangeConfiguration((e) => {
				if (
					...
					e.hasChanged(114 /* EditorOption.suggestFontSize */) ||
					e.hasChanged(115 /* EditorOption.suggestLineHeight */)
				) {
					configureFont();
				}
			})
	...
	renderElement(element, index, data) {
		...
		const color = [];
		if (
			completion.kind === 19 /* CompletionItemKind.Color */ &&
			...
			completion.kind === 20 /* CompletionItemKind.File */ &&
			...
			completion.kind === 23 /* CompletionItemKind.Folder */ &&
...
ItemRenderer = __decorate(
	[__param(1, IModelService), __param(2, ILanguageService), __param(3, IThemeService)],
	ItemRenderer
);
