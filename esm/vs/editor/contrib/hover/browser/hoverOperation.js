...
export class HoverOperation extends Disposable {
	constructor(_editor, _computer) {
		...
		this._firstWaitScheduler = this._register(
			new RunOnceScheduler(() => this._triggerAsyncComputation(), 0)
		);
		this._secondWaitScheduler = this._register(
			new RunOnceScheduler(() => this._triggerSyncComputation(), 0)
		);
		this._loadingMessageScheduler = this._register(
			...
			(() =>
				__awaiter(this, void 0, void 0, function* () {
					...
					try {
						try {
							for (
								var _d = true, _e = __asyncValues(this._asyncIterable), _f;
						...
						if (
							this._state === 3 /* HoverOperationState.WaitingForAsync */ ||
	...
	_triggerSyncComputation() {
		...
		this._setState(
			this._asyncIterableDone
				? 0 /* HoverOperationState.Idle */
				: 3 /* HoverOperationState.WaitingForAsync */
		);
	}
	...
	_fireResult() {
		if (
			this._state === 1 /* HoverOperationState.FirstWait */ ||
