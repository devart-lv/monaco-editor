...
export function getReindentEditOperations(
	...
	const indentationRules = languageConfigurationService.getLanguageConfiguration(
		model.getLanguageId()
	).indentationRules;
	if (!indentationRules) {
		return [];
...
export let AutoIndentOnPaste = class AutoIndentOnPaste {
	...
	update() {
		...
		// we are disabled
		if (
			this.editor.getOption(10 /* EditorOption.autoIndent */) <
				4 /* EditorAutoIndentStrategy.Full */ ||
			this.editor.getOption(53 /* EditorOption.formatOnPaste */)
		) {
			return;
		}
		// no model
