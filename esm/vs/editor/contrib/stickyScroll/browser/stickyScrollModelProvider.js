...
export let StickyModelProvider = class StickyModelProvider {
	constructor(_editor, _languageConfigurationService, _languageFeaturesService, defaultModel) {
		...
		const stickyModelFromCandidateOutlineProvider = new StickyModelFromCandidateOutlineProvider(
			_languageFeaturesService
		);
		const stickyModelFromSyntaxFoldingProvider = new StickyModelFromCandidateSyntaxFoldingProvider(
			this._editor,
			_languageFeaturesService
		);
	...
	update(textModel, textModelVersionId, token) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			this._cancelModelPromise();
			return yield this._updateScheduler.trigger(() =>
...
StickyModelProvider = __decorate(
	[__param(1, ILanguageConfigurationService), __param(2, ILanguageFeaturesService)],
	StickyModelProvider
);
...
let StickyModelFromCandidateOutlineProvider = class StickyModelFromCandidateOutlineProvider extends StickyModelCandidateProvider {
	...
	createStickyModel(textModel, modelVersionId, token, model) {
		...
		const { stickyOutlineElement, providerID } = this._stickyModelFromOutlineModel(
			model,
			(_a = this._stickyModel) === null || _a === void 0 ? void 0 : _a.outlineProviderId
		);
		...
		for (const child of outlineElement.children.values()) {
			if (child.symbol.selectionRange.startLineNumber !== child.symbol.range.endLineNumber) {
				if (child.symbol.selectionRange.startLineNumber !== previousStartLine) {
					children.push(
						this._stickyModelFromOutlineElement(child, child.symbol.selectionRange.startLineNumber)
					);
				} else {
	...
	_findSumOfRangesOfGroup(outline) {
		...
		if (outline instanceof OutlineElement) {
			return (
				res + outline.symbol.range.endLineNumber - outline.symbol.selectionRange.startLineNumber
			);
		} else {
			return res;
		}
