...
export let StickyScrollController = class StickyScrollController extends Disposable {
	constructor(
		...
		this._register(
			dom.addDisposableListener(
				this._stickyScrollWidget.getDomNode(),
	...
	_createClickLinkGesture() {
		...
		linkGestureStore.add(
			gesture.onMouseMoveOrRelevantKeyDown(([mouseEvent, _keyboardEvent]) => {
				...
				if (
					targetMouseEvent.detail === this._stickyScrollWidget.getId() &&
					...
					const stickyPositionProjectedOnEditor = new Range(
						lineNumber,
						column,
						lineNumber,
						column + text.length
	...
	_readConfiguration() {
		...
		if (options.enabled === false) {
			...
			this._sessionStore.add(
				this._stickyLineCandidateProvider.onDidChangeStickyScroll(() => this._renderStickyScroll())
			);
			this._enabled = true;
		}
		...
		if (lineNumberOption.renderType === 2 /* RenderLineNumbersType.Relative */) {
			this._sessionStore.add(
				this._editor.onDidChangeCursorPosition(() => this._renderStickyScroll())
			);
		}
	}
	_needsUpdate(event) {
	...
	_onDidResize() {
		...
		const theoreticalLines =
			layoutInfo.height / this._editor.getOption(64 /* EditorOption.lineHeight */);
	...
	findScrollWidgetState() {
		...
		const maxNumberStickyLines = Math.min(
			...
			this._editor.getOption(110 /* EditorOption.stickyScroll */).maxLineCount
		);
		...
		if (arrayVisibleRanges.length !== 0) {
			...
			const candidateRanges =
				this._stickyLineCandidateProvider.getCandidateStickyLinesIntersecting(fullVisibleRange);
