...
export let CodeLensContribution = class CodeLensContribution {
		...
		this._disposables.add(
			this._editor.onDidChangeConfiguration((e) => {
				if (
					...
					e.hasChanged(17 /* EditorOption.codeLensFontSize */) ||
					e.hasChanged(16 /* EditorOption.codeLensFontFamily */)
				) {
	...
	_getLayoutInfo() {
		const lineHeightFactor = Math.max(
			1.3,
			this._editor.getOption(64 /* EditorOption.lineHeight */) /
				this._editor.getOption(50 /* EditorOption.fontSize */)
		);
		let fontSize = this._editor.getOption(17 /* EditorOption.codeLensFontSize */);
		if (!fontSize || fontSize < 5) {
			fontSize = (this._editor.getOption(50 /* EditorOption.fontSize */) * 0.9) | 0;
		}
		return {
			fontSize,
				...
				const promises = request.map((request, i) => {
					if (!request.symbol.command && typeof request.provider.resolveCodeLens === 'function') {
						return Promise.resolve(
							request.provider.resolveCodeLens(model, request.symbol, token)
						).then((symbol) => {
