...
export let InlineCompletionsSource = class InlineCompletionsSource extends Disposable {
	constructor(
		...
		this.suggestWidgetInlineCompletions = disposableObservableValue(
	...
	clearSuggestWidgetInlineCompletions() {
		var _a;
		if (
			(_a = this.updateOperation.value) === null || _a === void 0
				? void 0
				: _a.request.context.selectedSuggestionInfo
		) {
			this.updateOperation.clear();
		}
	...
	update(position, context, activeInlineCompletion) {
		...
		if (
			(_a = this.updateOperation.value) === null || _a === void 0
				? void 0
				: _a.request.satisfies(request)
		) {
			return this.updateOperation.value.promise;
		} else if (
			(_b = target.get()) === null || _b === void 0 ? void 0 : _b.request.satisfies(request)
		) {
		...
		const promise = (() =>
			__awaiter(this, void 0, void 0, function* () {
				const shouldDebounce =
					updateOngoing || context.triggerKind === InlineCompletionTriggerKind.Automatic;
				if (shouldDebounce) {
				...
				const updatedCompletions = yield provideInlineCompletions(
				...
				if (activeInlineCompletion) {
					...
					if (
						activeInlineCompletion.canBeReused(this.textModel, position) &&
...
InlineCompletionsSource = __decorate(
	[__param(3, ILanguageFeaturesService), __param(4, ILanguageConfigurationService)],
...
class UpdateRequest {
	...
	satisfies(other) {
		return (
			...
			equals(this.context.selectedSuggestionInfo, other.context.selectedSuggestionInfo, (v1, v2) =>
				v1.equals(v2)
			) &&
...
export class UpToDateInlineCompletions {
	...
	constructor(inlineCompletionProviderResult, request, textModel, versionId) {
		...
		const ids = textModel.deltaDecorations(
			[],
	...
	prepend(inlineCompletion, range, addRefToSource) {
		...
		this._inlineCompletions.unshift(
			new InlineCompletionWithUpdatedRange(inlineCompletion, id, this.rangeVersion, range)
		);
	...
	dispose() {
		this.refCount--;
		if (this.refCount === 0) {
			this.textModel.deltaDecorations(
...
export class InlineCompletionWithUpdatedRange {
	constructor(inlineCompletion, decorationId, rangeVersion, initialRange) {
		...
		this._isValid = true;
		this._updatedRange =
			initialRange !== null && initialRange !== void 0 ? initialRange : inlineCompletion.range;
	}
	...
	isVisible(model, cursorPosition, reader) {
		...
		if (
			...
			!this.inlineCompletion.range
				.getStartPosition()
				.equals(this.getUpdatedRange(reader).getStartPosition()) ||
			cursorPosition.lineNumber !== minimizedReplacement.range.startLineNumber
		) {
			return false;
		}
		const originalValue = model
			.getValueInRange(minimizedReplacement.range, 1 /* EndOfLinePreference.LF */)
			.toLowerCase();
