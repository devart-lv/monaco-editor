...
import {
	...
	transaction
} from '../../../../base/common/observableImpl/base.js';
...
import {
	AudioCue,
	IAudioCueService
} from '../../../../platform/audioCues/browser/audioCueService.js';
...
export let InlineCompletionsController = class InlineCompletionsController extends Disposable {
	...
	constructor(
		...
		this.suggestWidgetAdaptor = this._register(
				...
				() => {
					...
					return (_b =
						(_a = this.model.get()) === null || _a === void 0
							? void 0
							: _a.selectedInlineCompletion.get()) === null || _b === void 0
						? void 0
						: _b.toSingleTextEdit(undefined);
				},
		...
		this.ghostTextWidget = this._register(
			this.instantiationService.createInstance(GhostTextWidget, this.editor, {
		...
		this._debounceValue = this.debounceService.for(
		...
		const enabled = observableFromEvent(
			editor.onDidChangeConfiguration,
			() => editor.getOption(60 /* EditorOption.inlineSuggest */).enabled
		);
		this._register(
			Event.runAndSubscribe(editor.onDidChangeModel, () => {
				transaction((tx) => {
					...
					if (textModel) {
						const model = instantiationService.createInstance(
							...
							observableFromEvent(
								editor.onDidChangeConfiguration,
								() => editor.getOption(113 /* EditorOption.suggest */).preview
							),
							observableFromEvent(
								editor.onDidChangeConfiguration,
								() => editor.getOption(113 /* EditorOption.suggest */).previewMode
							),
							observableFromEvent(
								editor.onDidChangeConfiguration,
								() => editor.getOption(60 /* EditorOption.inlineSuggest */).mode
							),
							enabled
						);
		...
		this._register(
			this.editor.onDidBlurEditorWidget(() => {
				...
				if (
					this.configurationService.getValue('editor.inlineSuggest.keepOnBlur') ||
		...
		this._register(
			autorun('forceRenderingAbove', (reader) => {
				const model = this.model.read(reader);
				const ghostText =
					model === null || model === void 0 ? void 0 : model.ghostText.read(reader);
		...
		this._register(
			autorun('play audio cue & read suggestion', (reader) => {
				const model = this.model.read(reader);
				const currentInlineCompletion =
					model === null || model === void 0 ? void 0 : model.selectedInlineCompletion.read(reader);
	...
	updateObservables(tx, changeReason) {
		...
		this.textModelVersionId.set(
			(_a = newModel === null || newModel === void 0 ? void 0 : newModel.getVersionId()) !== null &&
				_a !== void 0
				? _a
				: -1,
			tx,
		...
		this.cursorPosition.set(
			(_b = this.editor.getPosition()) !== null && _b !== void 0 ? _b : new Position(1, 1),
			tx
		);
	}
	shouldShowHoverAt(range) {
		var _a;
		const ghostText =
			(_a = this.model.get()) === null || _a === void 0 ? void 0 : _a.ghostText.get();
		if (ghostText) {
			return ghostText.parts.some((p) =>
				range.containsPosition(new Position(ghostText.lineNumber, p.column))
			);
		}
		return false;
	}
