...
export let GhostTextWidget = class GhostTextWidget extends Disposable {
	constructor(editor, model, languageService) {
		...
		this.uiState = derived('uiState', (reader) => {
			...
			const replacedRange =
				ghostText instanceof GhostTextReplacement ? ghostText.columnRange : undefined;
			const inlineTexts = [];
	...
	constructor(editor, languageIdCodec, lines) {
		...
		this.editorOptionsChanged = observableSignalFromEvent(
			...
			Event.filter(
				...
				(e) =>
					e.hasChanged(31 /* EditorOption.disableMonospaceOptimizations */) ||
		...
		this._register(
			autorun('update view zone', (reader) => {
...
const ttPolicy =
	(_a = window.trustedTypes) === null || _a === void 0
		? void 0
		: _a.createPolicy('editorGhostText', { createHTML: (value) => value });
