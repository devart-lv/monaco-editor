...
export let InlineCompletionsHoverParticipant = class InlineCompletionsHoverParticipant {
			...
			if (controller.shouldShowHoverAtViewZone(viewZoneData.viewZoneId)) {
				return new HoverForeignElementAnchor(
					...
					Range.fromPositions(
						this._editor
							.getModel()
							.validatePosition(viewZoneData.positionBefore || viewZoneData.position)
					),
	...
	renderHoverParts(context, hoverParts) {
		...
		const w = this._instantiationService.createInstance(
			...
			model.selectedInlineCompletion.map((v) => {
				var _a;
				return (_a =
					v === null || v === void 0
						? void 0
						: v.inlineCompletion.source.inlineCompletions.commands) !== null && _a !== void 0
					? _a
					: [];
			})
		);
	...
	renderScreenReaderText(context, part, disposableStore) {
		...
		const renderer = disposableStore.add(
			new MarkdownRenderer({ editor: this._editor }, this._languageService, this._openerService)
		);
		const render = (code) => {
			disposableStore.add(
		...
		disposableStore.add(
			autorun('update hover', (reader) => {
				var _a;
				const ghostText =
					(_a = part.controller.model.read(reader)) === null || _a === void 0
						? void 0
						: _a.ghostText.read(reader);
				if (ghostText) {
