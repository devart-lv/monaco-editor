	...
	run(accessor, editor, args) {
		...
		for (const selection of selections) {
			if (selection.isEmpty()) {
				...
				commands.push(
					new ReplaceCommandThatSelectsText(insertSelection, model.getValueInRange(selection))
				);
			}
		}
		editor.pushUndoStop();
...
class AbstractMoveLinesAction extends EditorAction {
	...
	run(accessor, editor) {
		...
		for (const selection of selections) {
			commands.push(
				new MoveLinesCommand(selection, this.down, autoIndent, languageConfigurationService)
			);
		}
		editor.pushUndoStop();
		editor.executeCommands(this.id, commands);
			...
			if (selection.isEmpty()) {
				if (selection.startColumn === 1) {
					...
					const deleteFromColumn =
						selection.startLineNumber === 1 ? 1 : model.getLineContent(deleteFromLine).length + 1;
	...
	run(_accessor, editor) {
		...
		for (let i = 0, len = reducedSelections.length; i < len; i++) {
			...
			const selectionEndPositionOffset =
				model.getLineContent(selection.endLineNumber).length - selection.endColumn;
...
TitleCaseAction.titleBoundary = new BackwardsCompatibleRegExp(
	"(^|[^\\p{L}\\p{N}']|((^|\\P{L})'))\\p{L}",
	'gmu'
);
...
SnakeCaseAction.singleLetters = new BackwardsCompatibleRegExp(
	'(\\p{Lu}|\\p{N})(\\p{Lu})(\\p{Ll})',
	'gmu'
);
export class CamelCaseAction extends AbstractCaseAction {
	...
	_modifyText(text, wordSeparators) {
		...
		return (
			firstWord +
			words.map((word) => word.substring(0, 1).toLocaleUpperCase() + word.substring(1)).join('')
		);
	}
}
...
KebabCaseAction.singleLetters = new BackwardsCompatibleRegExp(
	'(\\p{Lu}|\\p{N})(\\p{Lu}\\p{Ll})',
	'gmu'
);
