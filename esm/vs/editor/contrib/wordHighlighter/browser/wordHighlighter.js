...
class SemanticOccurenceAtPositionRequest extends OccurenceAtPositionRequest {
	...
	_compute(model, selection, wordSeparators, token) {
		return getOccurrencesAtPosition(this._providers, model, selection.getPosition(), token).then(
			(value) => value || []
		);
	}
}
...
class WordHighlighter {
	...
	_run() {
		...
		const workerRequestIsValid =
			this.workerRequest &&
			this.workerRequest.isValid(this.model, editorSelection, this.decorations);
		// There are 4 cases:
		...
		if (workerRequestIsValid) {
			...
			this.workerRequest = computeOccurencesAtPosition(
				...
				this.editor.getOption(125 /* EditorOption.wordSeparators */)
			);
...
WordHighlighterContribution = __decorate(
	[__param(1, IContextKeyService), __param(2, ILanguageFeaturesService)],
