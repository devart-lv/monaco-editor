...
class MessageWidget {
	...
	update(marker) {
		...
		let sourceAndCodeLength =
			((source === null || source === void 0 ? void 0 : source.length) || 0) + '()'.length;
		if (code) {
		...
		if (isNonEmptyArray(relatedInformation)) {
			...
			relatedInformationNode.style.paddingTop = `${Math.floor(
				this._editor.getOption(64 /* EditorOption.lineHeight */) * 0.66
			)}px`;
			this._lines += 1;
			for (const related of relatedInformation) {
				...
				relatedResource.innerText = `${this._labelService.getUriBasenameLabel(related.resource)}(${
...
export let MarkerNavigationWidget = class MarkerNavigationWidget extends PeekViewWidget {
	...
	showAtMarker(marker, markerIdx, markerCount) {
		...
		this._icon.className = `codicon ${SeverityIcon.className(
			MarkerSeverity.toSeverity(this._severity)
		)}`;
