...
export let GotoDefinitionAtPositionEditorContribution = class GotoDefinitionAtPositionEditorContribution {
	...
	startFindDefinitionFromMouse(mouseEvent, withKey) {
		...
		if (
			mouseEvent.target.type === 9 /* MouseTargetType.CONTENT_WIDGET */ &&
	...
	getPreviewValue(textEditorModel, startLineNumber, result) {
		...
		if (
			numberOfLinesInRange >= GotoDefinitionAtPositionEditorContribution.MAX_SOURCE_PREVIEW_LINES
		) {
	...
	isEnabled(mouseEvent, withKey) {
		...
		return (
			...
			!(
				((_a = mouseEvent.target.detail.injectedText) === null || _a === void 0
					? void 0
					: _a.options) instanceof ModelDecorationInjectedTextOptions
			) &&
	...
	gotoDefinition(position, openToSide) {
		...
		return this.editor.invokeWithinContext((accessor) => {
			const canPeek =
				!openToSide &&
				this.editor.getOption(84 /* EditorOption.definitionLinkOpensInPeek */) &&
