...
export class ClickLinkGesture extends Disposable {
	constructor(editor, alwaysFireOnMouseUp) {
			...
			this._editor.onDidChangeConfiguration((e) => {
				if (e.hasChanged(75 /* EditorOption.multiCursorModifier */)) {
					const newOpts = createOptions(
						this._editor.getOption(75 /* EditorOption.multiCursorModifier */)
					);
		...
		this._register(
			this._editor.onMouseUp((e) => this._onEditorMouseUp(new ClickLinkMouseEvent(e, this._opts)))
		);
		this._register(
		...
		this._register(
			this._editor.onKeyUp((e) => this._onEditorKeyUp(new ClickLinkKeyboardEvent(e, this._opts)))
		);
		this._register(this._editor.onMouseDrag(() => this._resetHandler()));
		this._register(
			this._editor.onDidChangeCursorSelection((e) => this._onDidChangeCursorSelection(e))
		);
	...
	_onEditorMouseDown(mouseEvent) {
		...
		this._lineNumberOnMouseDown = mouseEvent.target.position
			? mouseEvent.target.position.lineNumber
			: 0;
	}
	_onEditorMouseUp(mouseEvent) {
		const currentLineNumber = mouseEvent.target.position
			? mouseEvent.target.position.lineNumber
			: 0;
		if (
			...
			this._alwaysFireExecuteOnMouseUp
		) {
