	...
	_getNextMatch() {
		...
		const nextMatch = this._editor
			...
			.findNextMatch(
				...
				false,
				this.matchCase,
				this.wholeWord ? this._editor.getOption(125 /* EditorOption.wordSeparators */) : null,
				false
			);
		if (!nextMatch) {
			return null;
		...
		if (findState.isRevealed && findState.searchString.length > 0 && findState.isRegex) {
			...
			if (findState.searchScope) {
				matches = editorModel.findMatches(
					...
					findState.matchCase,
					findState.wholeWord
						? this._editor.getOption(125 /* EditorOption.wordSeparators */)
						: null,
					false,
					1073741824 /* Constants.MAX_SAFE_SMALL_INTEGER */
				);
		...
		super({
			...
			kbOpts: {
				...
				primary: KeyChord(
					...
					2048 /* KeyMod.CtrlCmd */ | 34 /* KeyCode.KeyD */
				),
				weight: 100 /* KeybindingWeight.EditorContrib */
			}
		});
...
export let SelectionHighlighter = class SelectionHighlighter extends Disposable {
	...
	static _createState(oldState, isEnabled, editor) {
		...
		return new SelectionHighlighterState(
			...
			r.searchText,
			r.matchCase,
			r.wholeWord ? editor.getOption(125 /* EditorOption.wordSeparators */) : null,
			oldState
		);
	}
	_setState(newState) {
		...
		const hasSemanticHighlights =
			...
			this.editor.getOption(78 /* EditorOption.occurrencesHighlight */);
...
SelectionHighlighter = __decorate([__param(1, ILanguageFeaturesService)], SelectionHighlighter);
