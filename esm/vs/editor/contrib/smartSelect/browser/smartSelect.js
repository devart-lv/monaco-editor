...
SmartSelectController = __decorate([__param(1, ILanguageFeaturesService)], SmartSelectController);
		...
		super(true, {
			...
			kbOpts: {
				...
				mac: {
					...
					secondary: [
						256 /* KeyMod.WinCtrl */ | 1024 /* KeyMod.Shift */ | 17 /* KeyCode.RightArrow */
					]
				},
		...
		super(false, {
			...
			kbOpts: {
				...
				mac: {
					...
					secondary: [
						256 /* KeyMod.WinCtrl */ | 1024 /* KeyMod.Shift */ | 15 /* KeyCode.LeftArrow */
					]
				},
				weight: 100 /* KeybindingWeight.EditorContrib */
			},
...
export function provideSelectionRanges(registry, model, positions, options, token) {
	return __awaiter(this, void 0, void 0, function* () {
		const providers = registry.all(model).concat(new WordSelectionRangeProvider()); // ALWAYS have word based selection range
		...
		for (const provider of providers) {
			work.push(
				Promise.resolve(provider.provideSelectionRanges(model, positions, token)).then(
					(allProviderRanges) => {
						if (
