...
export class FindWidget extends Widget {
	...
	_onStateChanged(e) {
		...
		if (e.isReplaceRevealed) {
			if (this._state.isReplaceRevealed) {
				if (
					!this._codeEditor.getOption(87 /* EditorOption.readOnly */) &&
	...
	_reveal() {
		...
		if (!this._isVisible) {
			...
			if (
				this._codeEditor.getOption(39 /* EditorOption.find */).seedSearchStringFromSelection &&
				selection
			) {
	...
	_buildDomNode() {
		...
		this._prevBtn = this._register(
			new SimpleButton({
				label:
					NLS_PREVIOUS_MATCH_BTN_LABEL + this._keybindingLabelFor(FIND_IDS.PreviousMatchFindAction),
...
registerThemingParticipant((theme, collector) => {
	...
	if (widgetBorderColor) {
		collector.addRule(
			`.monaco-editor .find-widget { border-left: 1px solid ${widgetBorderColor}; border-right: 1px solid ${widgetBorderColor}; border-bottom: 1px solid ${widgetBorderColor}; }`
		);
	}
