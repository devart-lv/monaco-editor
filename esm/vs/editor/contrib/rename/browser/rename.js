...
let RenameController = class RenameController {
	...
	constructor(
		...
		this._renameInputField = this._disposableStore.add(
	...
	run() {
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			const cts1 = new EditorStateCancellationTokenSource(
				this.editor,
				4 /* CodeEditorStateFlag.Position */ | 1 /* CodeEditorStateFlag.Value */,
				undefined,
				this._cts.token
			);
			let loc;
			try {
				...
				(_a = MessageController.get(this.editor)) === null || _a === void 0
					? void 0
					: _a.showMessage(
							e ||
								nls.localize(
			...
			if (loc.rejectReason) {
				(_b = MessageController.get(this.editor)) === null || _b === void 0
					? void 0
					: _b.showMessage(loc.rejectReason, position);
				return undefined;
			}
			...
			const inputFieldResult = yield this._renameInputField.getInput(
				loc.range,
				loc.text,
				selectionStart,
				selectionEnd,
			...
			const renameOperation = raceCancellation(
				...
				.then(
					(renameResult) =>
						__awaiter(this, void 0, void 0, function* () {
							...
							this.editor.setSelection(
								Range.fromPositions(this.editor.getSelection().getPosition())
							);
							this._bulkEditService
								...
								.catch((err) => {
									this._notificationService.error(
										nls.localize('rename.failedApply', 'Rename failed to apply edits')
									);
					...
					(err) => {
						this._notificationService.error(
							nls.localize('rename.failed', 'Rename failed to compute edits')
						);
						this._logService.error(err);
					}
				)
