...
export let DropIntoEditorController = class DropIntoEditorController extends Disposable {
	...
	constructor(
		...
		this._postDropWidgetManager = this._register(
			instantiationService.createInstance(PostDropWidgetManager, editor)
		);
		this._register(
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			const p = createCancelablePromise((token) =>
				__awaiter(this, void 0, void 0, function* () {
					...
					this._dropProgressManager.setAtPosition(
						position,
					...
					try {
						...
						const possibleDropEdits = yield raceCancellation(
							Promise.all(
								providers.map((provider) => {
									return provider.provideDocumentOnDropEdits(
										model,
										position,
										ourDataTransfer,
										tokenSource.token
						...
						tokenSource.dispose();
						if (
							((_b = this._currentOperation) === null || _b === void 0 ? void 0 : _b.id) ===
							operationId
						) {
							this._dropProgressManager.clear();
				...
				if (Array.isArray(data)) {
					for (const id of data) {
						const treeDataTransfer =
							yield this._treeViewsDragAndDropService.removeDragOperationTransfer(id.identifier);
						if (treeDataTransfer) {
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			const snippet =
				typeof edit.insertText === 'string'
					? SnippetParser.escape(edit.insertText)
			...
			const combinedWorkspaceEdit = {
				edits: [
					...
					...((_b = (_a = edit.additionalEdit) === null || _a === void 0 ? void 0 : _a.edits) !==
						null && _b !== void 0
						? _b
						: [])
				]
				...
				[
					{
						range: Range.fromPositions(position),
						options: {
							description: 'drop-line-suffix',
							stickiness: 0 /* TrackedRangeStickiness.AlwaysGrowsWhenTypingAtEdges */
						}
					}
				]
			);
			const editResult = yield this._bulkEditService.apply(combinedWorkspaceEdit, {
				editor,
				token
			});
			...
			if (editResult.isApplied && allEdits.length > 1) {
				...
				if (options.showDropSelector === 'afterDrop') {
					this._postDropWidgetManager.show(
						editRange !== null && editRange !== void 0 ? editRange : Range.fromPositions(position),
						{
...
registerEditorCommand(
	new (class extends EditorCommand {
		...
		runEditorCommand(_accessor, editor, _args) {
			var _a;
			(_a = DropIntoEditorController.get(editor)) === null || _a === void 0
				? void 0
				: _a.changeDropType();
		}
	})()
