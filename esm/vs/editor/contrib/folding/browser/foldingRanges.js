...
export class FoldingRegions {
	...
	toString() {
		...
		for (let i = 0; i < this.length; i++) {
			res[i] = `[${foldSourceAbbr[this.getSource(i)]}${
				this.isCollapsed(i) ? '+' : '-'
			}] ${this.getStartLineNumber(i)}/${this.getEndLineNumber(i)}`;
		}
		return res.join(', ');
	}
	toFoldRange(index) {
	...
	static sanitizeAndMerge(rangesA, rangesB, maxLineNumber) {
		maxLineNumber =
			maxLineNumber !== null && maxLineNumber !== void 0 ? maxLineNumber : Number.MAX_VALUE;
		...
		while (nextA || nextB) {
			...
			if (nextB && (!nextA || nextA.startLineNumber >= nextB.startLineNumber)) {
				...
				while (true) {
					...
					if (
						prescanB.source === 1 /* FoldSource.userDefined */ &&
