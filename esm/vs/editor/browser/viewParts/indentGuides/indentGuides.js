...
export class IndentGuidesOverlay extends DynamicViewOverlay {
	...
	onCursorStateChanged(e) {
		...
		if (
			!((_a = this._primaryPosition) === null || _a === void 0 ? void 0 : _a.equals(newPosition))
		) {
	...
	prepareRender(ctx) {
		...
		for (
			...
			const leftOffset =
				(_b =
					(_a = ctx.visibleRangeForPosition(new Position(lineNumber, 1))) === null || _a === void 0
						? void 0
						: _a.left) !== null && _b !== void 0
					? _b
					: 0;
...
registerThemingParticipant((theme, collector) => {
	...
	const colorValues = colors
		.map((c) => {
			...
			const effectiveGuideColor = transparentToUndefined(
				(_a = transparentToUndefined(guideColor)) !== null && _a !== void 0
					? _a
					: bracketColor === null || bracketColor === void 0
					? void 0
					: bracketColor.transparent(0.3)
			);
			const effectiveGuideColorActive = transparentToUndefined(
				(_b = transparentToUndefined(guideColorActive)) !== null && _b !== void 0
					? _b
					: bracketColor
			);
	...
	if (colorValues.length > 0) {
		for (let level = 0; level < 30; level++) {
			...
			collector.addRule(
				`.monaco-editor .${colorProvider
					.getInlineClassNameOfLevel(level)
					.replace(/ /g, '.')} { --guide-color: ${colors.guideColor}; --guide-color-active: ${
					colors.guideColorActive
				}; }`
			);
		}
		collector.addRule(
