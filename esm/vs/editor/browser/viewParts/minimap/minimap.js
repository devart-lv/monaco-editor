	...
	static create(
		...
		if (options.minimapHeightIsEditorHeight) {
			let logicalScrollHeight =
				realLineCount * options.lineHeight + options.paddingTop + options.paddingBottom;
...
export class Minimap extends ViewPart {
	...
	revealLineNumber(lineNumber) {
		...
		this._context.viewModel.revealRange(
			...
			1 /* viewEvents.VerticalRevealType.Center */,
			0 /* ScrollType.Smooth */
		);
	}
...
class InnerMinimap extends Disposable {
		...
		this._pointerDownListener = dom.addStandardDisposableListener(
			...
			(e) => {
				...
				const internalOffsetY =
					(this._model.options.canvasInnerHeight / this._model.options.canvasOuterHeight) *
					e.offsetY;
	...
	onLinesDeleted(deleteFromLineNumber, deleteToLineNumber) {
		var _a;
		(_a = this._lastRenderData) === null || _a === void 0
			? void 0
			: _a.onLinesDeleted(deleteFromLineNumber, deleteToLineNumber);
		return true;
	}
	onLinesInserted(insertFromLineNumber, insertToLineNumber) {
		var _a;
		(_a = this._lastRenderData) === null || _a === void 0
			? void 0
			: _a.onLinesInserted(insertFromLineNumber, insertToLineNumber);
		return true;
	}
	onScrollChanged() {
		this._renderDecorations = true;
	...
	_renderSelectionLineHighlights(
		canvasContext,
		selections,
		highlightedLines,
		layout,
	...
	_renderDecorationsLineHighlights(
		canvasContext,
		decorations,
		highlightedLines,
		layout,
	...
	static _renderUntouchedLines(
		...
		for (let lineNumber = startLineNumber; lineNumber <= endLineNumber; lineNumber++) {
			...
			const source_dy =
				lastLineIndex >= 0 && lastLineIndex < lastLinesLength ? lastLines[lastLineIndex].dy : -1;
			if (source_dy === -1) {
