...
export class MinimapCharRendererFactory {
	...
	static createSampleData(fontFamily) {
		...
		canvas.style.width =
			96 /* Constants.CHAR_COUNT */ * 10 /* Constants.SAMPLED_CHAR_WIDTH */ + 'px';
		...
		return ctx.getImageData(
			0,
			0,
			96 /* Constants.CHAR_COUNT */ * 10 /* Constants.SAMPLED_CHAR_WIDTH */,
			16 /* Constants.SAMPLED_CHAR_HEIGHT */
		);
	}
	/**
		...
		for (let y = 0; y < height; y++) {
			...
			for (let x = 0; x < width; x++) {
				...
				for (let sy = sourceY1; sy < sourceY2; sy++) {
					const sourceRow =
						sourceOffset + Math.floor(sy) * 3840; /* Constants.RGBA_SAMPLED_ROW_WIDTH */
	...
	static _downsample(data, scale) {
		const pixelsPerCharacter =
			2 /* Constants.BASE_CHAR_HEIGHT */ * scale * 1 /* Constants.BASE_CHAR_WIDTH */ * scale;
