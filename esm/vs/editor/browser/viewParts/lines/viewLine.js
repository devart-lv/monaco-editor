	...
	renderLine(lineNumber, deltaTop, viewportData, sb) {
		...
		if (isHighContrast(options.themeType) || this._options.renderWhitespace === 'selection') {
			...
			for (const selection of selections) {
				...
				const startColumn =
					selection.startLineNumber === lineNumber ? selection.startColumn : lineData.minColumn;
				const endColumn =
					selection.endLineNumber === lineNumber ? selection.endColumn : lineData.maxColumn;
...
class FastRenderedViewLine {
	constructor(domNode, renderLineInput, characterMapping) {
		...
		const keyColumnCount = Math.floor(
			renderLineInput.lineContent.length / 300 /* Constants.MaxMonospaceDistance */
		);
		if (keyColumnCount > 0) {
	...
	getWidthIsFast() {
		return (
			this.input.lineContent.length < 300 /* Constants.MaxMonospaceDistance */ ||
			this._cachedWidth !== -1
		);
	}
	monospaceAssumptionsAreValid() {
	...
	_getColumnPixelOffset(lineNumber, column, context) {
		...
		const keyColumnOrdinal =
			Math.floor((column - 1) / 300 /* Constants.MaxMonospaceDistance */) - 1;
	...
	getColumnOfNodeOffset(lineNumber, spanNode, offset) {
		...
		return this._characterMapping.getColumn(
