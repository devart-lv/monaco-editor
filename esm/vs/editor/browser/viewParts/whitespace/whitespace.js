	...
	_applyRenderWhitespace(ctx, lineNumber, selections, lineData) {
		...
		for (let charIndex = fauxIndentLength; charIndex < len; charIndex++) {
			...
			if (
				...
				const prevChCode =
					charIndex - 1 >= 0 ? lineContent.charCodeAt(charIndex - 1) : 0 /* CharCode.Null */;
				const nextChCode =
			...
			if (USE_SVG) {
				...
				if (chCode === 9 /* CharCode.Tab */) {
					...
					result += `<circle cx="${(visibleRange.left + spaceWidth / 2).toFixed(2)}" cy="${(
						lineHeight / 2
					).toFixed(2)}" r="${(spaceWidth / 7).toFixed(2)}" />`;
				}
			} else {
				if (chCode === 9 /* CharCode.Tab */) {
					...
					result += `<div class="mwh" style="left:${
						visibleRange.left
					}px;height:${lineHeight}px;">${String.fromCharCode(renderSpaceCharCode)}</div>`;
				}
			}
		}
		if (USE_SVG) {
