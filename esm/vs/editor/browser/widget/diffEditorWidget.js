...
class VisualEditorState {
	...
	apply(editor, overviewRuler, newDecorations, restoreScrollState) {
		...
		editor.changeViewZones((viewChangeAccessor) => {
			...
			for (let i = 0, length = newDecorations.zones.length; i < length; i++) {
				...
				if (newDecorations.zones[i].diff && viewZone.marginDomNode) {
					...
					if (
						((_a = newDecorations.zones[i].diff) === null || _a === void 0
							? void 0
							: _a.originalModel.getValueLength()) !== 0
					) {
		...
		// decorations
		editor.changeDecorations((changeAccessor) => {
			this._decorations = changeAccessor.deltaDecorations(
				this._decorations,
				newDecorations.decorations
			);
		});
		// overview ruler
		overviewRuler === null || overviewRuler === void 0
			? void 0
...
const diffInsertIcon = registerIcon(
	'diff-insert',
	Codicon.add,
	nls.localize('diffInsertIcon', 'Line decoration for inserts in the diff editor.')
);
const diffRemoveIcon = registerIcon(
	'diff-remove',
	Codicon.remove,
	nls.localize('diffRemoveIcon', 'Line decoration for removals in the diff editor.')
);
const ttPolicy =
	(_a = window.trustedTypes) === null || _a === void 0
		? void 0
		: _a.createPolicy('diffEditorWidget', { createHTML: (value) => value });
const ariaNavigationTip = nls.localize(
...
export let DiffEditorWidget = class DiffEditorWidget extends Disposable {
	constructor(
		...
		this._documentDiffProvider = this._register(
			instantiationService.createInstance(WorkerBasedDocumentDiffProvider, options)
		);
		this._register(
			this._documentDiffProvider.onDidChange((e) => this._beginUpdateDecorationsSoon())
		);
		...
		this._elementSizeObserver = this._register(
			new ElementSizeObserver(this._containerDomElement, options.dimension)
		);
		...
		if (this._options.renderSideBySide) {
			this._setStrategy(
				new DiffEditorWidgetSideBySide(
					...
					this._options.splitViewDefaultRatio
				)
			);
		} else {
			this._setStrategy(
				new DiffEditorWidgetInline(this._createDataSource(), this._options.enableSplitViewResizing)
			);
		}
		this._register(
		...
		this._register(
			editor.onMouseDown((event) => {
				...
				if (
					...
					((_a = event.target.element) === null || _a === void 0
						? void 0
						: _a.className.includes('arrow-revert-change'))
				) {
					const lineNumber = event.target.position.lineNumber;
					const viewZone = event.target;
					const change =
						(_b = this._diffComputationResult) === null || _b === void 0
							? void 0
							: _b.changes.find(
									(c) =>
										// delete change
										(viewZone === null || viewZone === void 0
											? void 0
	...
	revertChange(change) {
		...
		const originalRange =
			change.originalEndLineNumber > 0
				? new Range(
						...
						original.getLineMaxColumn(change.originalEndLineNumber)
				  )
				: null;
		...
		if (change.originalEndLineNumber === 0 && newRange) {
			...
			if (change.modifiedStartLineNumber > 1) {
				...
				range = newRange.setStartPosition(
					change.modifiedStartLineNumber - 1,
					modified.getLineMaxColumn(change.modifiedStartLineNumber - 1)
				);
			...
			const insertAt =
				change.modifiedStartLineNumber < modified.getLineCount()
					? new Position(change.modifiedStartLineNumber + 1, 1)
					: new Position(
							change.modifiedStartLineNumber,
							modified.getLineMaxColumn(change.modifiedStartLineNumber)
					  );
			editor.executeEdits('diffEditor', [
				{
	...
	updateOptions(_newOptions) {
		...
		const beginUpdateDecorations =
			changed.ignoreTrimWhitespace || changed.renderIndicators || changed.renderMarginRevertIcon;
		const beginUpdateDecorationsSoon =
			this._isVisible && (changed.maxComputationTime || changed.maxFileSize);
		...
		this._strategy.setEnableSplitViewResizing(
			this._options.enableSplitViewResizing,
			this._options.splitViewDefaultRatio
		);
		// renderSideBySide
	...
	revealLineInCenterIfOutsideViewport(
		lineNumber,
		scrollType = 0 /* editorCommon.ScrollType.Smooth */
	) {
	...
	_layoutOverviewRulers() {
		...
		const freeSpace =
			DiffEditorWidget.ENTIRE_DIFF_OVERVIEW_WIDTH - 2 * DiffEditorWidget.ONE_OVERVIEW_WIDTH;
	...
	_beginUpdateDecorations() {
		...
		this._documentDiffProvider
			.computeDiff(currentOriginalModel, currentModifiedModel, {
						...
						this._diffComputationResult = {
							...
							changes: result.changes.map((m) => {
								...
								return {
									...
									charChanges:
										innerChanges === null || innerChanges === void 0
											? void 0
											: innerChanges.map((m) => ({
													originalStartLineNumber: m.originalRange.startLineNumber,
	...
	_updateDecorations() {
		...
		const renderMarginRevertIcon =
			this._options.renderMarginRevertIcon &&
			!this._modifiedEditor.getOption(87 /* EditorOption.readOnly */);
		const diffDecorations = this._strategy.getEditorsDiffDecorations(
			lineChanges,
			this._options.ignoreTrimWhitespace,
			this._options.renderIndicators,
	...
	_doLayout() {
		...
		this._modifiedEditor.layout({
			width:
				width -
				splitPoint -
				(this._options.renderOverviewRuler ? DiffEditorWidget.ENTIRE_DIFF_OVERVIEW_WIDTH : 0),
			height: height - reviewHeight
		});
		if (this._originalOverviewRuler || this._modifiedOverviewRuler) {
			this._layoutOverviewRulers();
	...
	_getEquivalentLineForOriginalLineNumber(lineNumber) {
		...
		return (
			modifiedEquivalentLineNumber + lineChangeModifiedLength - lineChangeOriginalLength + delta
		);
	}
	_getEquivalentLineForModifiedLineNumber(lineNumber) {
		...
		return (
			originalEquivalentLineNumber + lineChangeOriginalLength - lineChangeModifiedLength + delta
		);
	}
...
class DiffEditorWidgetStyle extends Disposable {
	...
	applyColors(theme) {
		...
		const hasChanges =
			!newInsertColor.equals(this._insertColor) || !newRemoveColor.equals(this._removeColor);
...
class ViewZonesComputer {
	...
	getViewZones() {
		...
		const originalHasWrapping =
			this._originalEditor.getOption(140 /* EditorOption.wrappingInfo */).wrappingColumn !== -1;
		const modifiedHasWrapping =
			this._modifiedEditor.getOption(140 /* EditorOption.wrappingInfo */).wrappingColumn !== -1;
			...
			if (hasWrapping) {
				...
				for (let i = 0; i < count; i++) {
					...
					const originalViewLineCount =
						originalCoordinatesConverter.getModelLineViewLineCount(originalLineNumber);
					const modifiedViewLineCount =
						modifiedCoordinatesConverter.getModelLineViewLineCount(modifiedLineNumber);
		...
		for (const lineChange of lineChanges) {
			...
			if (renderMarginRevertIcon) {
				if (lineChange.modifiedEndLineNumber > 0) {
					...
					const viewZone = zones.modified.find(
						(z) => z.afterLineNumber === lineChange.modifiedStartLineNumber
					);
					if (viewZone) {
