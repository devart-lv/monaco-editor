...
export class TextAreaHandler extends ViewPart {
	constructor(context, viewController, visibleRangeProvider) {
				...
				if (this._accessibilitySupport === 1 /* AccessibilitySupport.Disabled */) {
					...
					if (
						...
						const text = simpleModel.getValueInRange(
							selection,
							0 /* EndOfLinePreference.TextDefined */
						);
		...
		this._textAreaInput = this._register(
			new TextAreaInput(textAreaInputHost, textAreaWrapper, platform.OS, browser)
		);
		this._register(
		...
		this._register(
			this._textAreaInput.onCompositionStart((e) => {
				...
				this._context.viewModel.revealRange(
					...
					0 /* viewEvents.VerticalRevealType.Simple */,
	...
	_getWordBeforePosition(position) {
		...
		const wordSeparators = getMapForWordSeparators(
			this._context.configuration.options.get(125 /* EditorOption.wordSeparators */)
		);
	...
	_setAccessibilityOptions(options) {
		...
		if (
			this._accessibilitySupport === 2 /* AccessibilitySupport.Enabled */ &&
		...
		if (
			wrappingColumn !== -1 &&
			this._accessibilitySupport !== 1 /* AccessibilitySupport.Disabled */
		) {
	...
	onConfigurationChanged(e) {
		...
		this.textArea.setAttribute('tabindex', String(options.get(119 /* EditorOption.tabIndex */)));
		if (
			e.hasChanged(32 /* EditorOption.domReadOnly */) ||
	...
	_ensureReadOnlyAttribute() {
		...
		const useReadOnly =
			!IME.enabled ||
			(options.get(32 /* EditorOption.domReadOnly */) &&
				options.get(87 /* EditorOption.readOnly */));
		if (useReadOnly) {
	...
	_render() {
		...
		if (this._visibleTextArea) {
			...
			if (
				...
				const lineCount = this._newlinecount(
					this.textArea.domNode.value.substr(0, this.textArea.domNode.selectionStart)
				);
		...
		if (platform.isMacintosh) {
			...
			const lineCount =
				(_a = this._textAreaInput.textAreaState.newlineCountBeforeSelection) !== null &&
				_a !== void 0
					? _a
					: this._newlinecount(
							this.textArea.domNode.value.substr(0, this.textArea.domNode.selectionStart)
