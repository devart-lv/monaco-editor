...
class HitTestRequest extends BareHitTestRequest {
	...
	toString() {
		return `pos(${this.pos.x},${this.pos.y}), editorPos(${this.editorPos.x},${
			this.editorPos.y
		}), relativePos(${this.relativePos.x},${this.relativePos.y}), mouseVerticalOffset: ${
			this.mouseVerticalOffset
		}, mouseContentHorizontalOffset: ${this.mouseContentHorizontalOffset}\n\ttarget: ${
			this.target ? this.target.outerHTML : null
		}`;
	}
...
export class MouseTargetFactory {
	...
	static _createMouseTarget(ctx, request, domHitTestExecuted) {
		...
		result =
			result || MouseTargetFactory._hitTestViewLines(ctx, resolvedRequest, domHitTestExecuted);
		...
		if (request.isInMarginArea) {
			...
			if (offset <= ctx.layoutInfo.glyphMarginWidth) {
				...
				return request.fulfillMargin(
					2 /* MouseTargetType.GUTTER_GLYPH_MARGIN */,
					pos,
					res.range,
					detail
				);
			...
			if (offset <= ctx.layoutInfo.lineNumbersWidth) {
				// On the line numbers
				return request.fulfillMargin(
					3 /* MouseTargetType.GUTTER_LINE_NUMBERS */,
					pos,
					res.range,
					detail
				);
			...
			return request.fulfillMargin(
				4 /* MouseTargetType.GUTTER_LINE_DECORATIONS */,
				pos,
				res.range,
				detail
			);
	...
	getMouseColumn(relativePos) {
		...
		return MouseTargetFactory._getMouseColumn(
			...
			options.get(48 /* EditorOption.fontInfo */).typicalHalfwidthCharacterWidth
		);
	}
	...
	static _doHitTestWithCaretRangeFromPoint(ctx, request) {
		...
		if (!isBelowLastLine) {
			...
			let adjustedPageY =
				request.pos.y + (lineCenteredVerticalOffset - request.mouseVerticalOffset);
	...
	static _actualDoHitTestWithCaretRangeFromPoint(ctx, coords) {
		...
		if (startContainer.nodeType === startContainer.TEXT_NODE) {
			...
			const parent3ClassName =
				parent3 && parent3.nodeType === parent3.ELEMENT_NODE ? parent3.className : null;
			...
			const parent2ClassName =
				parent2 && parent2.nodeType === parent2.ELEMENT_NODE ? parent2.className : null;
	...
	static _doHitTestWithCaretPositionFromPoint(ctx, coords) {
		...
		if (hitResult.offsetNode.nodeType === hitResult.offsetNode.ELEMENT_NODE) {
			...
			const parent1ClassName =
				parent1 && parent1.nodeType === parent1.ELEMENT_NODE ? parent1.className : null;
			...
			if (parent1ClassName === ViewLine.CLASS_NAME) {
				...
				const tokenSpan =
					hitResult.offsetNode.childNodes[
						Math.min(hitResult.offset, hitResult.offsetNode.childNodes.length - 1)
					];
				if (tokenSpan) {
