...
export class MouseHandler extends ViewEventHandler {
	constructor(context, viewController, viewHelper) {
		...
		this._register(
			dom.addDisposableListener(this.viewHelper.viewDomNode, dom.EventType.POINTER_UP, (e) => {
	...
	_onMouseDown(e, pointerId) {
		...
		const targetIsContent =
			t.type === 6 /* MouseTargetType.CONTENT_TEXT */ ||
			t.type === 7 /* MouseTargetType.CONTENT_EMPTY */;
		const targetIsGutter =
			t.type === 2 /* MouseTargetType.GUTTER_GLYPH_MARGIN */ ||
		...
		const selectOnLineNumbers = this._context.configuration.options.get(
			104 /* EditorOption.selectOnLineNumbers */
		);
		const targetIsViewZone =
			t.type === 8 /* MouseTargetType.CONTENT_VIEW_ZONE */ ||
...
class MouseDownOperation extends Disposable {
	...
	_onMouseDownThenMove(e) {
		...
		if (this._mouseState.isDragAndDrop) {
			...
			if (
				position.type === 13 /* MouseTargetType.OUTSIDE_EDITOR */ &&
	...
	start(targetType, e, pointerId) {
		...
		this._mouseState.setStartedOnLineNumbers(
			targetType === 3 /* MouseTargetType.GUTTER_LINE_NUMBERS */
		);
	...
	_dispatchMouse(position, inSelectionMode, revealType) {
		...
		this._viewController.dispatchMouse({
			...
			onInjectedText:
				position.type === 6 /* MouseTargetType.CONTENT_TEXT */ &&
...
class TopBottomDragScrollingOperation extends Disposable {
	...
	_getScrollSpeed() {
		...
		const viewportInLines =
			this._context.configuration.options.get(139 /* EditorOption.layoutInfo */).height /
			lineHeight;
	...
	_execute() {
		...
		const scrollValue =
			this._position.outsidePosition === 'above' ? -scrollInPixels : scrollInPixels;
