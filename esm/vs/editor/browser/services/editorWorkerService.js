...
export let EditorWorkerService = class EditorWorkerService extends Disposable {
	...
	computedUnicodeHighlights(uri, options, range) {
		return this._workerManager
			.withWorker()
			.then((client) => client.computedUnicodeHighlights(uri, options, range));
	}
	computeDiff(original, modified, options, algorithm) {
		return __awaiter(this, void 0, void 0, function* () {
			const result = yield this._workerManager
				.withWorker()
				.then((client) => client.computeDiff(original, modified, options, algorithm));
			if (!result) {
				return null;
			}
	...
	computeMoreMinimalEdits(resource, edits, pretty = false) {
		if (isNonEmptyArray(edits)) {
			...
			const result = this._workerManager
				.withWorker()
				.then((client) => client.computeMoreMinimalEdits(resource, edits, pretty));
			result.finally(() =>
...
class WordBasedCompletionItemProvider {
	...
	provideCompletionItems(model, position) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			const data = yield client.textualSuggest(
				models,
...
export class EditorWorkerClient extends Disposable {
	...
	computeDiff(original, modified, options, algorithm) {
		return this._withSyncedResources([original, modified], /* forceLargeModels */ true).then(
			(proxy) => {
