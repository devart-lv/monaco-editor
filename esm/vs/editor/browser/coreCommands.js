...
(function (CoreNavigationCommands) {
	...
	class LineEndCommand extends CoreEditorCommand {
		...
		_exec(viewModel, cursors) {
			...
			for (let i = 0, len = cursors.length; i < len; i++) {
				...
				result[i] = CursorState.fromModelState(
					cursor.modelState.move(this._inSelectionMode, lineNumber, maxColumn, 0)
				);
			}
			return result;
		}
	...
	class EditorScrollImpl extends CoreEditorCommand {
		...
		determineScrollMethod(args) {
			...
			const verticalUnits = [
				1 /* EditorScroll_.Unit.Line */, 2 /* EditorScroll_.Unit.WrappedLine */,
				3 /* EditorScroll_.Unit.Page */, 4 /* EditorScroll_.Unit.HalfPage */,
				5 /* EditorScroll_.Unit.Editor */, 6 /* EditorScroll_.Unit.Column */
			];
			const horizontalDirections = [
				4 /* EditorScroll_.Direction.Left */, 2 /* EditorScroll_.Direction.Right */
			];
			const verticalDirections = [
				1 /* EditorScroll_.Direction.Up */, 3 /* EditorScroll_.Direction.Down */
			];
		...
		_computeDesiredScrollTop(viewModel, args) {
			if (args.unit === 1 /* EditorScroll_.Unit.Line */) {
				...
				const visibleModelRange =
					viewModel.coordinatesConverter.convertViewRangeToModelRange(visibleViewRange);
			...
			const deltaLines =
				(args.direction === 1 /* EditorScroll_.Direction.Up */ ? -1 : 1) * noOfLines;
			return (
				viewModel.viewLayout.getCurrentScrollTop() + deltaLines * viewModel.cursorConfig.lineHeight
			);
		}
		...
		_computeDesiredScrollLeft(viewModel, args) {
			const deltaColumns =
				(args.direction === 4 /* EditorScroll_.Direction.Left */ ? -1 : 1) * args.value;
			return (
	...
	CoreNavigationCommands.ScrollEditorTop = registerEditorCommand(
		new (class extends CoreEditorCommand {
			constructor() {
				super({
					id: 'scrollEditorTop',
	...
	CoreNavigationCommands.ScrollEditorBottom = registerEditorCommand(
		new (class extends CoreEditorCommand {
			constructor() {
				super({
	...
	CoreNavigationCommands.ScrollLeft = registerEditorCommand(
		new (class extends CoreEditorCommand {
			constructor() {
				super({
					id: 'scrollLeft',
	...
	CoreNavigationCommands.ScrollRight = registerEditorCommand(
		new (class extends CoreEditorCommand {
			constructor() {
				super({
					id: 'scrollRight',
	...
	CoreNavigationCommands.RevealLine = registerEditorCommand(
		new (class extends CoreEditorCommand {
			...
			runCoreEditorCommand(viewModel, args) {
				...
				let lineNumber =
					typeof lineNumberArg === 'number' ? lineNumberArg + 1 : parseInt(lineNumberArg) + 1;
				if (lineNumber < 1) {
					lineNumber = 1;
				}
