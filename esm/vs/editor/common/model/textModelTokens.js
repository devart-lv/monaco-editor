...
export class TokenizerWithStateStoreAndTextModel extends TokenizerWithStateStore {
	...
	tokenizeLineWithEdit(position, length, newText) {
		...
		const newLineContent =
			curLineContent.substring(0, column - 1) +
			newText +
			curLineContent.substring(column - 1 + length);
	...
	isCheapToTokenize(lineNumber) {
		...
		if (
			...
			this._textModel.getLineLength(lineNumber) <
				2048 /* Constants.CHEAP_TOKENIZATION_LENGTH_LIMIT */
		) {
			return true;
		}
		return false;
...
export class TrackingTokenizationStateStore {
	...
	acceptChanges(changes) {
		for (const c of changes) {
			...
			this.acceptChange(
				new LineRange(c.range.startLineNumber, c.range.endLineNumber + 1),
				eolCount + 1
			);
		}
	}
