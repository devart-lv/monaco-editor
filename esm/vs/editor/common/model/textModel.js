...
export let TextModel = class TextModel extends Disposable {
	...
	constructor(
		...
		this._onDidChangeDecorations = this._register(
		...
		const bufferTextLength = this._buffer.getValueLengthInRange(
			...
			0 /* model.EndOfLinePreference.TextDefined */
		);
	...
	updateOptions(_newOpts) {
		this._assertNotDisposed();
		const tabSize =
			typeof _newOpts.tabSize !== 'undefined' ? _newOpts.tabSize : this._options.tabSize;
		const indentSize =
	...
	getOffsetAt(rawPosition) {
		...
		const position = this._validatePosition(
			...
			rawPosition.column,
			0 /* StringOffsetValidationType.Relaxed */
		);
	...
	_validateRangeRelaxedNoAllocations(range) {
		...
		let startLineNumber = Math.floor(
			typeof initialStartLineNumber === 'number' && !isNaN(initialStartLineNumber)
	...
	_isValidRange(range, validationType) {
		...
		if (
			!this._isValidPosition(endLineNumber, endColumn, 0 /* StringOffsetValidationType.Relaxed */)
		) {
			return false;
		}
		if (validationType === 1 /* StringOffsetValidationType.SurrogatePairs */) {
			const charCodeBeforeStart =
				startColumn > 1 ? this._buffer.getLineCharCode(startLineNumber, startColumn - 2) : 0;
			...
			if (editsAreNearCursors) {
				for (let i = 0, len = this._trimAutoWhitespaceLines.length; i < len; i++) {
					...
					if (allowTrimLine) {
						...
						editOperations.push(
							new model.ValidAnnotatedEditOperation(null, trimRange, null, false, false, false)
						);
					}
				}
			}
		...
		if (contentChanges.length !== 0) {
			...
			for (let i = 0, len = contentChanges.length; i < len; i++) {
				...
				const currentEditStartLineNumber =
					newLineCount - lineCount - changeLineCountDelta + startLineNumber;
				...
				const decorationsWithInjectedTextInEditedRange =
					this._decorationsTree.getInjectedTextInInterval(
						...
						this.getOffsetAt(
							new Position(lastInsertedLineNumber, this.getLineMaxColumn(lastInsertedLineNumber))
						),
						0
					);
	...
	deltaDecorations(oldDecorations, newDecorations, ownerId = 0) {
		...
		try {
			...
			if (this._deltaDecorationCallCnt > 1) {
				...
				onUnexpectedError(
					new Error(`Invoking deltaDecorations recursively could lead to leaking decorations.`)
				);
			}
			this._onDidChangeDecorations.beginDeferredEmit();
			return this._deltaDecorationsImpl(ownerId, oldDecorations, newDecorations);
	...
	getLinesDecorations(
		...
		pushMany(
			decorations,
			this._decorationProvider.getDecorationsInRange(range, ownerId, filterOutValidation)
		);
		return decorations;
	}
	...
	_changeDecorationOptionsImpl(decorationId, options) {
		...
		const nodeWasInOverviewRuler =
			node.options.overviewRuler && node.options.overviewRuler.color ? true : false;
...
export class ModelDecorationGlyphMarginOptions {
	constructor(options) {
		var _a;
		this.position =
			(_a = options === null || options === void 0 ? void 0 : options.position) !== null &&
			_a !== void 0
				? _a
				: model.GlyphMarginLane.Left;
	}
}
...
export class ModelDecorationOptions {
	...
	constructor(options) {
		...
		this.blockDoesNotCollapse =
			(_a = options.blockDoesNotCollapse) !== null && _a !== void 0 ? _a : null;
		...
		this.hideInCommentTokens =
			(_d = options.hideInCommentTokens) !== null && _d !== void 0 ? _d : false;
		this.hideInStringTokens =
			(_e = options.hideInStringTokens) !== null && _e !== void 0 ? _e : false;
	}
}
