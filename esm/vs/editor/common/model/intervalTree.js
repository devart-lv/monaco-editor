...
function setNodeColor(node, color) {
	node.metadata =
		(node.metadata & 254) /* Constants.ColorMaskInverse */ |
		(color << 0) /* Constants.ColorOffset */;
}
function getNodeIsVisited(node) {
	return (
		(node.metadata & 2) /* Constants.IsVisitedMask */ >>> 1 /* Constants.IsVisitedOffset */ === 1
	);
}
...
function getNodeIsForValidation(node) {
	return (
		(node.metadata & 4) /* Constants.IsForValidationMask */ >>>
			2 /* Constants.IsForValidationOffset */ ===
		1
	);
}
...
function getNodeIsInGlyphMargin(node) {
	return (
		(node.metadata & 64) /* Constants.IsMarginMask */ >>> 6 /* Constants.IsMarginOffset */ === 1
	);
}
function setNodeIsInGlyphMargin(node, value) {
	node.metadata =
		(node.metadata & 191) /* Constants.IsMarginMaskInverse */ |
		((value ? 1 : 0) << 6) /* Constants.IsMarginOffset */;
}
...
function _setNodeStickiness(node, stickiness) {
	node.metadata =
		(node.metadata & 231) /* Constants.StickinessMaskInverse */ |
		(stickiness << 3) /* Constants.StickinessOffset */;
}
function getCollapseOnReplaceEdit(node) {
	return (
		(node.metadata & 32) /* Constants.CollapseOnReplaceEditMask */ >>>
...
function adjustMarkerBeforeColumn(
	markerOffset,
	markerStickToPreviousCharacter,
	checkOffset,
	moveSemantics
) {
...
export function nodeAcceptEdit(node, start, end, textLength, forceMoveMarkers) {
	...
	const startStickToPreviousCharacter =
		nodeStickiness === 0 /* TrackedRangeStickiness.AlwaysGrowsWhenTypingAtEdges */ ||
		nodeStickiness === 2 /* TrackedRangeStickiness.GrowsOnlyWhenTypingBefore */;
	const endStickToPreviousCharacter =
		nodeStickiness === 1 /* TrackedRangeStickiness.NeverGrowsWhenTypingAtEdges */ ||
		nodeStickiness === 2 /* TrackedRangeStickiness.GrowsOnlyWhenTypingBefore */;
...
function noOverlapReplace(T, start, end, textLength) {
	...
	while (node !== SENTINEL) {
		...
		if (nodeStart > end) {
			...
			if (
				node.delta < -1073741824 /* Constants.MIN_SAFE_DELTA */ ||
				node.delta > 1073741824 /* Constants.MAX_SAFE_DELTA */
			) {
...
function rbTreeDelete(T, z) {
	...
	if (z.left === SENTINEL) {
		...
		x.delta += z.delta;
		if (
			x.delta < -1073741824 /* Constants.MIN_SAFE_DELTA */ ||
			x.delta > 1073741824 /* Constants.MAX_SAFE_DELTA */
		) {
			T.requestNormalizeDelta = true;
		}
		x.start += z.delta;
		...
		y.delta = z.delta;
		if (
			y.delta < -1073741824 /* Constants.MIN_SAFE_DELTA */ ||
			y.delta > 1073741824 /* Constants.MAX_SAFE_DELTA */
		) {
			T.requestNormalizeDelta = true;
		}
	}
	...
	while (x !== T.root && getNodeColor(x) === 0 /* NodeColor.Black */) {
		if (x === x.parent.left) {
			...
			if (
				getNodeColor(w.left) === 0 /* NodeColor.Black */ &&
				getNodeColor(w.right) === 0 /* NodeColor.Black */
			) {
