...
export class CursorsController extends Disposable {
	...
	restoreState(eventsCollector, states) {
		...
		this.revealPrimary(
			...
			true,
			1 /* editorCommon.ScrollType.Immediate */
		);
	}
		...
		if (event instanceof ModelInjectedTextChangedEvent) {
			...
			if (hadFlushEvent) {
				...
				this._emitStateChangedIfNecessary(
					eventsCollector,
					'model',
					1 /* CursorChangeReason.ContentFlush */,
					null,
					false
				);
			} else {
					...
					if (
						this.setStates(
							...
							e.isUndoing
								...
								: 2 /* CursorChangeReason.RecoverFromMarkers */,
							cursorState
						)
					) {
						this.revealPrimary(
							...
							0 /* VerticalRevealType.Simple */,
							true,
							0 /* editorCommon.ScrollType.Smooth */
						);
					}
				} else {
	...
	_executeEdit(
		...
		eventsCollector,
		source,
		cursorChangeReason = 0 /* CursorChangeReason.NotSet */
	) {
	...
	executeCommand(eventsCollector, command, source) {
		this._executeEdit(
			() => {
				...
				this._executeEditOperation(
					new EditOperationResult(0 /* EditOperationType.Other */, [command], {
	...
	executeCommands(eventsCollector, commands, source) {
		this._executeEdit(
			() => {
				this._executeEditOperation(
					new EditOperationResult(0 /* EditOperationType.Other */, commands, {
						shouldPushStackElementBefore: false,
						shouldPushStackElementAfter: false
					})
				);
	...
	static executeCommands(model, selectionsBefore, commands) {
		...
		for (let i = 0, len = ctx.trackedRanges.length; i < len; i++) {
			ctx.model._setTrackedRange(
				ctx.trackedRanges[i],
				null,
				0 /* TrackedRangeStickiness.AlwaysGrowsWhenTypingAtEdges */
			);
		}
		return result;
	}
