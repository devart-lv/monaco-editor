...
} from '../commands/surroundSelectionCommand.js';
...
export class TypeOperations {
	...
	static compositionType(
		...
		return new EditOperationResult(4 /* EditOperationType.TypingOther */, commands, {
			shouldPushStackElementBefore: shouldPushStackElementBetween(
				prevEditOperationType,
				4 /* EditOperationType.TypingOther */
			),
			shouldPushStackElementAfter: false
		});
	}
	static _compositionType(
		model,
		selection,
		text,
		replacePrevCharCnt,
	...
	static _enter(config, model, keepPosition, range) {
		...
		if (
			!model.tokenization.isCheapToTokenize(range.getStartPosition().lineNumber) ||
	...
	static _isAutoClosingOvertype(config, model, selections, autoClosedCharacters, ch) {
		...
		for (let i = 0, len = selections.length; i < len; i++) {
			...
			const beforeCharacter =
				position.column > 2 ? lineText.charCodeAt(position.column - 2) : 0; /* CharCode.Null */
	...
	static _getAutoClosingPairClose(config, model, selections, ch, chIsAlreadyTyped) {
		...
		for (const position of positions) {
			...
			if (
				!pair.shouldAutoClose(scopedLineTokens, beforeColumn - scopedLineTokens.firstCharOffset)
			) {
				return null;
			}
	...
	static _isSurroundSelectionType(config, model, selections, ch) {
		...
		for (const selection of selections) {
			...
			for (
				...
				const endIndex =
					lineNumber === selection.endLineNumber ? selection.endColumn - 1 : lineText.length;
	...
	static _typeInterceptorElectricChar(prevEditOperationType, config, model, selection, ch) {
		...
		if (electricAction.matchOpenBracket) {
			const endColumn =
				(lineTokens.getLineContent() + ch).lastIndexOf(electricAction.matchOpenBracket) + 1;
			...
			if (match) {
				...
				const lineFirstNonBlankColumn =
					model.getLineFirstNonWhitespaceColumn(position.lineNumber) || position.column;
...
function shouldPushStackElementBetween(previousTypingOperation, typingOperation) {
	...
	return (
		normalizeOperationType(previousTypingOperation) !== normalizeOperationType(typingOperation)
	);
}
...
function isTypingOperation(type) {
	return (
		type === 4 /* EditOperationType.TypingOther */ ||
		type === 5 /* EditOperationType.TypingFirstSpace */ ||
