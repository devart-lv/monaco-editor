		...
		if (cur.seq1Range.isEmpty) {
			...
			if (all) {
				...
				result[result.length - 1] = new SequenceDiff(
					...
					new OffsetRange(lastResult.seq2Range.start, cur.seq2Range.endExclusive - length)
				);
				continue;
			}
		}
...
export function shiftSequenceDiffs(sequence1, sequence2, sequenceDiffs) {
	...
	for (let i = 0; i < sequenceDiffs.length; i++) {
		...
		if (diff.seq1Range.isEmpty) {
			...
			const seq2NextStart =
				i + 1 < sequenceDiffs.length ? sequenceDiffs[i + 1].seq2Range.start : sequence2.length;
			...
			const seq1NextStart =
				i + 1 < sequenceDiffs.length ? sequenceDiffs[i + 1].seq1Range.start : sequence1.length;
			sequenceDiffs[i] = shiftDiffToBetterPosition(
				diff.reverse(),
				sequence2,
				sequence1,
