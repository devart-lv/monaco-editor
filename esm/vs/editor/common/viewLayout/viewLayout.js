...
export class ViewLayout extends Disposable {
	constructor(configuration, lineCount, scheduleAtNextAnimationFrame) {
		...
		this._linesLayout = new LinesLayout(
			lineCount,
			options.get(64 /* EditorOption.lineHeight */),
			padding.top,
			padding.bottom
		);
		...
		this._scrollable.setScrollDimensions(
			new EditorScrollDimensions(layoutInfo.contentWidth, 0, layoutInfo.height, 0)
		);
	...
	_configureSmoothScrollDuration() {
		this._scrollable.setSmoothScrollDuration(
			this._configuration.options.get(109 /* EditorOption.smoothScrolling */)
	...
	_getContentHeight(width, height, contentWidth) {
		...
		if (options.get(100 /* EditorOption.scrollBeyondLastLine */)) {
			result += Math.max(
				...
				height -
					options.get(64 /* EditorOption.lineHeight */) -
					options.get(81 /* EditorOption.padding */).bottom
			);
		} else {
	...
	_computeContentWidth(maxLineWidth) {
		...
		if (wrappingInfo.isViewportWrapping) {
			...
			const extraHorizontalSpace =
				options.get(99 /* EditorOption.scrollBeyondLastColumn */) *
