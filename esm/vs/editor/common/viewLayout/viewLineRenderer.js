...
export class LineRange {
	...
	equals(otherLineRange) {
		return (
			this.startOffset === otherLineRange.startOffset && this.endOffset === otherLineRange.endOffset
		);
	}
}
...
export class CharacterMapping {
	static getPartIndex(partData) {
		return (
			(partData & 4294901760) /* CharacterMappingConstants.PART_INDEX_MASK */ >>>
			16 /* CharacterMappingConstants.PART_INDEX_OFFSET */
		);
	}
	static getCharIndex(partData) {
		return (
			(partData & 65535) /* CharacterMappingConstants.CHAR_INDEX_MASK */ >>>
	...
	if (input.lineContent.length === 0) {
		if (input.lineDecorations.length > 0) {
			...
			for (const lineDecoration of input.lineDecorations) {
				if (
					lineDecoration.type === 1 /* InlineDecorationType.Before */ ||
...
function resolveRenderLineInput(input) {
	...
	if (
		input.renderWhitespace === 4 /* RenderWhitespace.All */ ||
		...
		if (charIndex < firstNonWhitespaceIndex || charIndex > lastNonWhitespaceIndex) {
			...
			if (onlyBoundary) {
				...
				if (wasInWhitespace) {
					...
					const nextChCode =
						charIndex + 1 < len ? lineContent.charCodeAt(charIndex + 1) : 0 /* CharCode.Null */;
					isInWhitespace =
						nextChCode === 32 /* CharCode.Space */ || nextChCode === 9 /* CharCode.Tab */;
				}
			} else {
				isInWhitespace = true;
			}
				...
				if (generateLinePartForEachWhitespace) {
					...
					for (let i = lastEndIndex + 1; i <= charIndex; i++) {
						result[resultLen++] = new LinePart(
							i,
							'mtkw',
							1 /* LinePartMetadata.IS_WHITESPACE */,
							false
						);
					}
				} else {
	...
	if (wasInWhitespace) {
		...
		if (continuesWithWrappedLine && onlyBoundary) {
			...
			const isSingleTrailingSpace =
				lastCharCode === 32 /* CharCode.Space */ &&
...
function _renderLine(input, sb) {
	...
	for (let partIndex = 0, tokensLen = parts.length; partIndex < tokensLen; partIndex++) {
		...
		const partRendersWhitespace =
			renderWhitespace !== 0 /* RenderWhitespace.None */ && part.isWhitespace();
		...
		if (partRendersWhitespace) {
			let partWidth = 0;
			{
				...
				for (; _charIndex < partEndIndex; _charIndex++) {
					...
					const charWidth =
						(charCode === 9 /* CharCode.Tab */ ? tabSize - (_visibleColumn % tabSize) : 1) | 0;
					partWidth += charWidth;
	...
	if (isOverflowing) {
		...
		sb.appendString(
			nls.localize('showMore', 'Show more ({0})', renderOverflowingCharCount(overflowingCharCount))
		);
		sb.appendString('</span>');
	}
	sb.appendString('</span>');
