...
class ModelLineProjection {
	...
	getViewLineContent(model, modelLineNumber, outputLineIndex) {
		...
		if (this._projectionData.injectionOffsets !== null) {
			const injectedTexts = this._projectionData.injectionOffsets.map(
				(offset, idx) =>
					new LineInjectedText(0, 0, offset + 1, this._projectionData.injectionOptions[idx], 0)
			);
	...
	getViewLinesData(
		...
		if (injectionOffsets) {
			lineWithInjections = model.tokenization.getLineTokens(modelLineNumber).withInserted(
	...
	_getViewLineData(lineWithInjections, inlineDecorations, outputLineIndex) {
		...
		const startVisibleColumn =
			outputLineIndex === 0 ? 0 : lineBreakData.breakOffsetsVisibleColumn[outputLineIndex - 1];
