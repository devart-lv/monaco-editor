...
export class MonospaceLineBreaksComputerFactory {
	static create(options) {
		return new MonospaceLineBreaksComputerFactory(
			options.get(128 /* EditorOption.wordWrapBreakBeforeCharacters */),
			options.get(127 /* EditorOption.wordWrapBreakAfterCharacters */)
		);
	}
...
function createLineBreaksFromPreviousLineBreaks(
	...
	while (prevIndex < prevLen) {
		...
		if (prevBreakOffsetVisibleColumn <= breakingColumn) {
			...
			let prevCharCode =
				prevBreakOffset === 0 ? 0 /* CharCode.Null */ : lineText.charCodeAt(prevBreakOffset - 1);
			let prevCharCodeClass =
				prevBreakOffset === 0 ? 0 /* CharacterClass.NONE */ : classifier.get(prevCharCode);
	...
	if (wrappingIndent !== 0 /* WrappingIndent.None */) {
		...
		if (firstNonWhitespaceIndex !== -1) {
			...
			for (let i = 0; i < firstNonWhitespaceIndex; i++) {
				const charWidth =
					lineText.charCodeAt(i) === 9 /* CharCode.Tab */
