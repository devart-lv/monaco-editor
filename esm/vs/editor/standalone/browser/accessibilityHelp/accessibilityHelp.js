...
const CONTEXT_ACCESSIBILITY_WIDGET_VISIBLE = new RawContextKey(
...
let AccessibilityHelpWidget = class AccessibilityHelpWidget extends Widget {
	constructor(editor, _contextKeyService, _keybindingService, _openerService) {
		...
		const heading = append(
			...
			$('h1', undefined, AccessibilityHelpNLS.accessibilityHelpTitle)
		);
		...
		this._register(
			addStandardDisposableListener(this._contentDomNode.domNode, 'keydown', (e) => {
				if (!this._isVisible) {
					return;
				}
