...
export class TokenizationSupportAdapter {
	...
	_toBinaryTokens(languageIdCodec, tokens) {
		...
		for (let i = 0, len = tokens.length; i < len; i++) {
			const t = tokens[i];
			const metadata =
				tokenTheme.match(languageId, t.scopes) | 1024; /* MetadataConsts.BALANCED_BRACKETS_MASK */
...
export function registerTokensProviderFactory(languageId, factory) {
	const adaptedFactory = new languages.LazyTokenizationSupport(() =>
...
export function registerCodeActionProvider(languageSelector, provider, metadata) {
	...
	return languageFeaturesService.codeActionProvider.register(languageSelector, {
		providedCodeActionKinds:
			metadata === null || metadata === void 0 ? void 0 : metadata.providedCodeActionKinds,
