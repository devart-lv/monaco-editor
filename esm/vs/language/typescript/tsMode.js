		...
		const relatedUris = diagnostics
			...
			.map((relatedInformation) =>
				relatedInformation.file
					? monaco_editor_core_exports.Uri.parse(relatedInformation.file.fileName)
					: null
			);
				...
				return {
					...
					kind:
						highlightSpans.kind === 'writtenReference'
							? monaco_editor_core_exports.languages.DocumentHighlightKind.Write
							: monaco_editor_core_exports.languages.DocumentHighlightKind.Text
				};
			});
		});
	}
	...
	async provideDocumentSymbols(model, token) {
		...
		const convert = (item, containerLabel) => {
			const result2 = {
				...
				detail: '',
				kind:
					outlineTypeTable[item.kind] || monaco_editor_core_exports.languages.SymbolKind.Variable,
