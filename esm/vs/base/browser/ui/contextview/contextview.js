...
export class ContextView extends Disposable {
	constructor(container, domPosition) {
		...
		this._register(
			toDisposable(() => this.setContainer(null, 1 /* ContextViewDOMPosition.ABSOLUTE */))
		);
	}
	...
	doLayout() {
		...
		const anchorAxisAlignment =
			this.delegate.anchorAxisAlignment || 0; /* AnchorAxisAlignment.VERTICAL */
		let top;
		let left;
		if (anchorAxisAlignment === 0 /* AnchorAxisAlignment.VERTICAL */) {
			const verticalAnchor = {
				...
				size: around.height,
				position:
					anchorPosition === 0 /* AnchorPosition.BELOW */
		...
		this.view.style.top = `${
			top -
			(this.useFixedPosition ? DOM.getDomNodePagePosition(this.view).top : containerPosition.top)
		}px`;
		this.view.style.left = `${
			left -
			(this.useFixedPosition ? DOM.getDomNodePagePosition(this.view).left : containerPosition.left)
		}px`;
