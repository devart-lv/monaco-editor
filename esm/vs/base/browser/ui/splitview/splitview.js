...
export class SplitView extends Disposable {
	...
	constructor(container, options = {}) {
		...
		this.orientation =
			(_a = options.orientation) !== null && _a !== void 0 ? _a : 0 /* Orientation.VERTICAL */;
		this.inverseAltBehavior =
			(_b = options.inverseAltBehavior) !== null && _b !== void 0 ? _b : false;
		this.proportionalLayout =
			(_c = options.proportionalLayout) !== null && _c !== void 0 ? _c : true;
		this.getSashOrthogonalSize = options.getSashOrthogonalSize;
		this.el = document.createElement('div');
		this.el.classList.add('monaco-split-view2');
		this.el.classList.add(
			this.orientation === 0 /* Orientation.VERTICAL */ ? 'vertical' : 'horizontal'
		);
				...
				this.viewContainer,
				{
					vertical:
						this.orientation === 0 /* Orientation.VERTICAL */
							? (_d = options.scrollbarVisibility) !== null && _d !== void 0
								? _d
								: 1 /* ScrollbarVisibility.Auto */
							: 2 /* ScrollbarVisibility.Hidden */,
					horizontal:
						this.orientation === 1 /* Orientation.HORIZONTAL */
							? (_e = options.scrollbarVisibility) !== null && _e !== void 0
								? _e
								: 1 /* ScrollbarVisibility.Auto */
							: 2 /* ScrollbarVisibility.Hidden */
				},
	...
	layout(size, layoutContext) {
		...
		if (!this.proportions) {
			...
			const lowPriorityIndexes = indexes.filter(
				(i) => this.viewItems[i].priority === 1 /* LayoutPriority.Low */
			);
			const highPriorityIndexes = indexes.filter(
				(i) => this.viewItems[i].priority === 2 /* LayoutPriority.High */
			);
			this.resize(
	...
	onSashStart({ sash, start, alt }) {
		...
		const disposable = combinedDisposable(
			addDisposableListener(document.body, 'keydown', (e) =>
				resetSashDragState(this.sashDragState.current, e.altKey)
			),
			addDisposableListener(document.body, 'keyup', () =>
	...
	onSashChange({ current }) {
		const { index, start, sizes, alt, minDelta, maxDelta, snapBefore, snapAfter } =
			this.sashDragState;
			...
			const sash =
				this.orientation === 0 /* Orientation.VERTICAL */
					...
					: new Sash(
							...
							Object.assign(Object.assign({}, opts), { orientation: 0 /* Orientation.VERTICAL */ })
					  );
	...
	updateSashEnablement() {
		...
		for (let index = 0; index < this.sashItems.length; index++) {
			...
			if (min && max) {
				...
				const snapAfterIndex = this.findFirstSnapIndex(downIndexes);
				const snappedBefore =
					typeof snapBeforeIndex === 'number' && !this.viewItems[snapBeforeIndex].visible;
				const snappedAfter =
					typeof snapAfterIndex === 'number' && !this.viewItems[snapAfterIndex].visible;
