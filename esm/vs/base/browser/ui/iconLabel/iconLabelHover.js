...
export function setupNativeHover(htmlElement, tooltip) {
	if (isString(tooltip)) {
		...
		tooltip === null || tooltip === void 0 ? void 0 : tooltip.markdownNotSupportedFallback
	) {
	...
	update(content, focus, options) {
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (content === undefined || isString(content) || content instanceof HTMLElement) {
				...
				resolvedContent =
					(_a = content.markdown) !== null && _a !== void 0
						? _a
						: content.markdownNotSupportedFallback;
			} else {
...
export function setupCustomHover(hoverDelegate, htmlElement, content, options) {
	...
	const hideHover = (disposeWidget, disposePreparation) => {
		...
		if (disposePreparation) {
			hoverPreparation === null || hoverPreparation === void 0
				? void 0
				: hoverPreparation.dispose();
			hoverPreparation = undefined;
		}
		if (hadHover) {
			(_a = hoverDelegate.onDidHideHover) === null || _a === void 0
				? void 0
				: _a.call(hoverDelegate);
		}
	};
	...
	const onMouseOver = () => {
		...
		toDispose.add(
			dom.addDisposableListener(htmlElement, dom.EventType.MOUSE_LEAVE, onMouseLeave, true)
		);
		const onMouseDown = () => hideHover(true, true);
		toDispose.add(
			dom.addDisposableListener(htmlElement, dom.EventType.MOUSE_DOWN, onMouseDown, true)
		);
		const target = {
		...
		if (hoverDelegate.placement === undefined || hoverDelegate.placement === 'mouse') {
			...
			toDispose.add(
				dom.addDisposableListener(htmlElement, dom.EventType.MOUSE_MOVE, onMouseMove, true)
			);
		}
		toDispose.add(triggerShowHover(hoverDelegate.delay, false, target));
		hoverPreparation = toDispose;
	};
	const mouseOverDomEmitter = dom.addDisposableListener(
		htmlElement,
	...
	const hover = {
		...
		update: (newContent, hoverOptions) =>
			__awaiter(this, void 0, void 0, function* () {
				content = newContent;
				yield hoverWidget === null || hoverWidget === void 0
					? void 0
					: hoverWidget.update(content, undefined, hoverOptions);
			}),
		dispose: () => {
