...
class Trait {
	...
	splice(start, deleteCount, elements) {
		...
		if (this.sortedIndexes.length > 0 && sortedIndexes.length === 0 && length > 0) {
			const first =
				(_a = this.sortedIndexes.find((index) => index >= start)) !== null && _a !== void 0
					? _a
					: length - 1;
...
class KeyboardController {
	get onKeyDown() {
		return this.disposables.add(
			Event.chain(this.disposables.add(new DomEmitter(this.view.domNode, 'keydown')).event)
	...
	constructor(list, view, options) {
		...
		this.onKeyDown
			.filter((e) => e.keyCode === 16 /* KeyCode.UpArrow */)
			.on(this.onUpArrow, this, this.disposables);
		this.onKeyDown
			.filter((e) => e.keyCode === 18 /* KeyCode.DownArrow */)
			.on(this.onDownArrow, this, this.disposables);
		this.onKeyDown
			.filter((e) => e.keyCode === 11 /* KeyCode.PageUp */)
			.on(this.onPageUpArrow, this, this.disposables);
		this.onKeyDown
			.filter((e) => e.keyCode === 12 /* KeyCode.PageDown */)
			.on(this.onPageDownArrow, this, this.disposables);
		this.onKeyDown
			.filter((e) => e.keyCode === 9 /* KeyCode.Escape */)
			.on(this.onEscape, this, this.disposables);
		if (options.multipleSelectionSupport !== false) {
			this.onKeyDown
				.filter(
					(e) =>
						(platform.isMacintosh ? e.metaKey : e.ctrlKey) && e.keyCode === 31 /* KeyCode.KeyA */
				)
...
export const DefaultKeyboardNavigationDelegate = new (class {
	mightProducePrintableCharacter(event) {
		...
		return (
			(event.keyCode >= 31 /* KeyCode.KeyA */ && event.keyCode <= 56) /* KeyCode.KeyZ */ ||
...
class TypeNavigationController {
	...
	updateOptions(options) {
		...
		this.mode =
			(_b = options.typeNavigationMode) !== null && _b !== void 0
				? _b
	...
	enable() {
		...
		const onChar = this.enabledDisposables
			.add(
				Event.chain(this.enabledDisposables.add(new DomEmitter(this.view.domNode, 'keydown')).event)
			)
			.filter((e) => !isInputElement(e.target))
...
class DOMFocusController {
	constructor(list, view) {
		...
		this.view = view;
		this.disposables = new DisposableStore();
		const onKeyDown = this.disposables
			.add(Event.chain(this.disposables.add(new DomEmitter(view.domNode, 'keydown')).event))
			.filter((e) => !isInputElement(e.target))
			.map((e) => new StandardKeyboardEvent(e));
		onKeyDown
			.filter(
				(e) =>
					e.keyCode === 2 /* KeyCode.Tab */ && !e.ctrlKey && !e.metaKey && !e.shiftKey && !e.altKey
			)
	...
	style(styles) {
		...
		const focusAndSelectionOutline = asCssValueWithDefault(
			...
			asCssValueWithDefault(
				...
				(_a = styles.listFocusOutline) !== null && _a !== void 0 ? _a : ''
			)
		);
		...
		const inactiveFocusAndSelectionOutline = asCssValueWithDefault(
			styles.listSelectionOutline,
			(_b = styles.listInactiveFocusOutline) !== null && _b !== void 0 ? _b : ''
		);
...
class PipelineRenderer {
	...
	disposeElement(element, index, templateData, height) {
		...
		let i = 0;
		for (const renderer of this.renderers) {
			(_a = renderer.disposeElement) === null || _a === void 0
				? void 0
				: _a.call(renderer, element, index, templateData[i], height);
			i += 1;
		}
	}
	disposeTemplate(templateData) {
...
class ListViewDragAndDrop {
	...
	onDragStart(data, originalEvent) {
		var _a, _b;
		(_b = (_a = this.dnd).onDragStart) === null || _b === void 0
			? void 0
			: _b.call(_a, data, originalEvent);
	}
	...
	onDragLeave(data, targetElement, targetIndex, originalEvent) {
		var _a, _b;
		(_b = (_a = this.dnd).onDragLeave) === null || _b === void 0
			? void 0
			: _b.call(_a, data, targetElement, targetIndex, originalEvent);
	}
	onDragEnd(originalEvent) {
		var _a, _b;
		(_b = (_a = this.dnd).onDragEnd) === null || _b === void 0
			? void 0
			: _b.call(_a, originalEvent);
	}
...
export class List {
	...
	get onContextMenu() {
		...
		const fromKeyDown = this.disposables
			.add(Event.chain(this.disposables.add(new DomEmitter(this.view.domNode, 'keydown')).event))
			.map((e) => new StandardKeyboardEvent(e))
			.filter(
				(e) =>
			...
			.filter(() => false).event;
		const fromKeyUp = this.disposables
			.add(Event.chain(this.disposables.add(new DomEmitter(this.view.domNode, 'keyup')).event))
			...
			.map(({ browserEvent }) => {
				...
				const anchor =
					typeof index !== 'undefined' ? this.view.domElement(index) : this.view.domNode;
				return { index, element, anchor, browserEvent };
			}).event;
		const fromMouse = this.disposables
			.add(Event.chain(this.view.onContextMenu))
	...
	constructor(user, container, virtualDelegate, renderers, _options = DefaultOptions) {
		...
		if (this.accessibilityProvider) {
			...
			(_c = (_b = this.accessibilityProvider).onDidChangeActiveDescendant) === null || _c === void 0
				? void 0
				: _c.call(_b, this.onDidChangeActiveDescendant, this, this.disposables);
		}
		...
		if (_options.keyboardNavigationLabelProvider) {
			...
			this.typeNavigationController = new TypeNavigationController(
				...
				(_d = _options.keyboardNavigationEventFilter) !== null && _d !== void 0 ? _d : () => true,
				delegate
			);
	...
	updateOptions(optionsUpdate = {}) {
		...
		(_a = this.typeNavigationController) === null || _a === void 0
			? void 0
			: _a.updateOptions(this._options);
		...
		(_b = this.keyboardController) === null || _b === void 0
			? void 0
			: _b.updateOptions(optionsUpdate);
	...
	focusPreviousPage(browserEvent, filter) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (
				...
				(currentlyFocusedElementIndex === undefined ||
					currentlyFocusedElementIndex >= firstPageIndex)
			) {
