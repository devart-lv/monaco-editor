...
export function renderMarkdown(markdown, options = {}, markedOptions = {}) {
	...
	if (options.actionHandler) {
		...
		options.actionHandler.disposables.add(
			DOM.addDisposableListener(element, 'keydown', (e) => {
				...
				if (
					!keyboardEvent.equals(10 /* KeyCode.Space */) &&
					!keyboardEvent.equals(3 /* KeyCode.Enter */)
				) {
					return;
				}
	...
	const markdownHtmlDoc = htmlParser.parseFromString(
		sanitizeRenderedMarkdown(markdown, renderedMarkdown),
		'text/html'
	);
	...
	if (codeBlocks.length > 0) {
		Promise.all(codeBlocks).then((tuples) => {
			...
			for (const placeholderElement of placeholderElements) {
				const renderedElement = renderedElements.get(
					(_a = placeholderElement.dataset['code']) !== null && _a !== void 0 ? _a : ''
				);
				if (renderedElement) {
		...
		for (const placeholderElement of placeholderElements) {
			const renderedElement = renderedElements.get(
				(_b = placeholderElement.dataset['code']) !== null && _b !== void 0 ? _b : ''
			);
			if (renderedElement) {
				DOM.reset(placeholderElement, renderedElement);
			}
...
function sanitizeRenderedMarkdown(options, renderedMarkdown) {
	...
	try {
		return dompurify.sanitize(
			renderedMarkdown,
			Object.assign(Object.assign({}, config), { RETURN_TRUSTED_TYPE: true })
		);
	} finally {
...
export function renderMarkdownAsPlaintext(markdown) {
	...
	const html = marked
		.parse(value, { renderer: plainTextRenderer.value })
