...
class RemovedResources {
	...
	createMessage() {
		...
		for (const [, element] of this.elements) {
			const dest =
				element.reason === 0 /* RemovedResourceReason.ExternalRemoval */
					? externalRemoval
					: noParallelUniverses;
...
export let UndoRedoService = class UndoRedoService {
	...
	removeElements(resource) {
		const strResource =
			typeof resource === 'string' ? resource : this.getUriComparisonKey(resource);
		if (this._editStacks.has(strResource)) {
			const editStack = this._editStacks.get(strResource);
			editStack.dispose();
...
UndoRedoService = __decorate(
	[__param(0, IDialogService), __param(1, INotificationService)],
	UndoRedoService
);
