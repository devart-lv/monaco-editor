	...
	update() {
		...
		if (this.togglesUpdated) {
			...
			const concreteToggles =
				(_b =
					(_a = this.toggles) === null || _a === void 0
						? void 0
						: _a.filter((opts) => opts instanceof Toggle)) !== null && _b !== void 0
					? _b
					: [];
...
QuickInput.noPromptMessage = localize(
	'inputModeEntry',
		...
		if (!this.visible) {
			...
			this.visibleDisposables.add(
				this.ui.list.onDidChangeSelection(({ items: selectedItems, event }) => {
					...
					if (selectedItems.length) {
						this.handleAccept(
							event instanceof MouseEvent && event.button === 1 /* mouse middle click */
						);
					}
				})
			);
	...
	update() {
		...
		if (this.valueSelectionUpdated) {
			...
			this.ui.inputBox.select(
				this._valueSelection && { start: this._valueSelection[0], end: this._valueSelection[1] }
			);
		}
...
export class QuickInputController extends Disposable {
	...
	getUI() {
		...
		const inputBox = this._register(
			new QuickInputBox(filterContainer, this.styles.inputBox, this.styles.toggle)
		);
		...
		this._register(
			list.onDidChangeFocus(() => {
				...
				inputBox.setAttribute(
					...
					(_a = list.getActiveDescendant()) !== null && _a !== void 0 ? _a : ''
				);
			})
		);
		this._register(
			...
			dom.addDisposableListener(
				...
				(e) => {
					this.previousFocusElement =
						e.relatedTarget instanceof HTMLElement ? e.relatedTarget : undefined;
				},
				true
			)
		);
	...
	pick(picks, options = {}, token = CancellationToken.None) {
		return new Promise((doResolve, reject) => {
			let resolve = (result) => {
				var _a;
				resolve = doResolve;
				(_a = options.onKeyMods) === null || _a === void 0
					? void 0
					: _a.call(options, input.keyMods);
				doResolve(result);
			};
			...
			const disposables = [
				...
				input.onDidTriggerSeparatorButton((event) => {
					var _a;
					return (_a = options.onDidTriggerSeparatorButton) === null || _a === void 0
						? void 0
	...
	hide(reason) {
		...
		if (controller) {
			...
			if (!focusChanged) {
				...
				if (
					currentElement === null || currentElement === void 0
						? void 0
	...
	updateStyles() {
		if (this.ui) {
			...
			this.ui.titleBar.style.backgroundColor =
				quickInputTitleBackground !== null && quickInputTitleBackground !== void 0
					? quickInputTitleBackground
					: '';
			this.ui.container.style.backgroundColor =
				quickInputBackground !== null && quickInputBackground !== void 0
					? quickInputBackground
					: '';
			this.ui.container.style.color =
				quickInputForeground !== null && quickInputForeground !== void 0
					? quickInputForeground
					: '';
			...
			if (this.styles.pickerGroup.pickerGroupForeground) {
				content.push(
					`.quick-input-list .quick-input-list-separator-as-item { color:  ${this.styles.pickerGroup.pickerGroupForeground}; }`
				);
			}
			if (
