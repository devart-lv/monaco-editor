...
export class ConfigurationModel {
	...
	inspect(section, overrideIdentifier) {
		...
		const override = overrideIdentifier
			? this.rawConfiguration.getOverrideValue(section, overrideIdentifier)
			: undefined;
		const merged = overrideIdentifier
			? this.rawConfiguration.override(overrideIdentifier).getValue(section)
			: value;
	...
	merge(...others) {
		...
		const raws = ((_a = this.raw) === null || _a === void 0 ? void 0 : _a.length)
			? [...this.raw]
			: [this];
		for (const other of others) {
			raws.push(
				...(((_b = other.raw) === null || _b === void 0 ? void 0 : _b.length) ? other.raw : [other])
			);
			if (other.isEmpty()) {
				continue;
			}
...
export class ConfigurationModelParser {
	...
	parseRaw(raw, options) {
		this._raw = raw;
		const { contents, keys, overrides, restricted, hasExcludedProperties } = this.doParseRaw(
			raw,
			options
		);
	...
	filter(properties, configurationProperties, filterOverriddenProperties, options) {
		...
		if (
			...
			!(options === null || options === void 0 ? void 0 : options.skipRestricted)
		) {
			...
			if (OVERRIDE_PROPERTY_REGEX.test(key) && filterOverriddenProperties) {
				...
				const scope = propertySchema
					? typeof propertySchema.scope !== 'undefined'
						...
						: 3 /* ConfigurationScope.WINDOW */
					: undefined;
				if (
					propertySchema === null || propertySchema === void 0 ? void 0 : propertySchema.restricted
				) {
					restricted.push(key);
				}
				...
				if (
					...
					!(
						...
						(propertySchema === null || propertySchema === void 0
							? void 0
...
class ConfigurationInspectValue {
	...
	get userInspectValue() {
		if (!this._userInspectValue) {
			this._userInspectValue = this.inspect(
	...
	get user() {
		return this.userInspectValue.value !== undefined || this.userInspectValue.override !== undefined
			? { value: this.userInspectValue.value, override: this.userInspectValue.override }
			: undefined;
	}
}
