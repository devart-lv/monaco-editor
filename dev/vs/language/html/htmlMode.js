	...
	var moduleExports = (() => {
		...
		var DiagnosticsAdapter = class {
			constructor(_languageId, _worker, configChangeEvent) {
				...
				this._disposables.push(
					monaco_editor_core_exports.editor.onWillDisposeModel(onModelRemoved)
				);
				this._disposables.push(
		...
		var DocumentHighlightAdapter = class {
			...
			provideDocumentHighlights(model, position, token) {
				...
				return this._worker(resource)
					.then((worker) =>
						worker.findDocumentHighlights(resource.toString(), fromPosition(position))
					)
					.then((entries) => {
						if (!entries) {
							return;
		...
		var SelectionRangeAdapter = class {
			...
			provideSelectionRanges(model, positions, token) {
				...
				return this._worker(resource)
					.then((worker) =>
						worker.getSelectionRanges(resource.toString(), positions.map(fromPosition))
					)
		...
		function setupMode1(defaults) {
			...
			monaco_editor_core_exports.languages.registerLinkProvider(
				languageId,
				new DocumentLinkAdapter(worker)
			);
